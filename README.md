...
`osl-app` is the first NekoSpice GUI alpha crate. It uses `eframe` with the `wgpu` renderer selected explicitly, opens the same Rust-native KiCad canvas scene used by `kicad-inspect` / `kicad-select`, draws the first schematic canvas view, supports pan, zoom, fit-to-scene, and click selection through `KicadCanvasScene::hit_test`, and keeps SVG rendering as a headless regression baseline rather than the GUI rendering path.
