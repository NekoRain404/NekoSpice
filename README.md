...
`osl-app` is the first NekoSpice GUI alpha crate. It uses `eframe` with the `wgpu` renderer selected explicitly, opens the same Rust-native KiCad canvas scene used by `kicad-inspect` / `kicad-select`, draws the first schematic canvas view, supports pan, zoom, fit-to-scene, viewport culling through `osl-kicad` item bounds/intersection helpers, click selection through `KicadCanvasScene::hit_test`, and delete/move/save for selected UUID-backed schematic items through `KicadSchematicEdit` plus the KiCad writer. After a GUI edit, selection is refreshed by `KicadCanvasScene::item_hit_by_uuid`, so the GUI does not need to know KiCad file layout or geometry internals. SVG rendering remains a headless regression baseline rather than the GUI rendering path.
