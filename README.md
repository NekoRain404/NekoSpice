...
`osl kicad-inspect <file.kicad_sch> --canvas` emits full canvas scene JSON for downstream GUI work, including symbols, symbol graphics, pins, wires, buses, labels, text items, junctions, no-connect markers, styles, and bounds. `KicadCanvasScene::to_summary_json()` remains available for compact counts.
