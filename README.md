...
- `nekospice`: hardware-accelerated GUI alpha using egui on wgpu, loading KiCad schematic canvas scenes through the same Rust-native IR and selection/edit backend.
...
cargo run -p osl-app --bin nekospice
```
...
`osl-app` is the first NekoSpice GUI alpha crate. It uses `eframe` with the `wgpu` renderer selected explicitly, opens the same Rust-native KiCad canvas scene used by `kicad-inspect` / `kicad-select`, and now uses a three-panel engineering layout: project/selection/property/tool state on the left, schematic canvas in the center, and a KiCad symbol-library browser on the right. The canvas supports pan, zoom, fit-to-scene, viewport culling through `osl-kicad` item bounds/intersection helpers, click selection through `KicadCanvasScene::hit_test`, delete/move/save for selected UUID-backed schematic items, symbol placement, basic wire / bus / bus-entry / local-global-hierarchical label / text / junction / no-connect / hierarchical sheet drawing tools, and selected symbol Reference / Value / mirror editing through `KicadSchematicEdit` plus the KiCad writer. The library browser loads `sym-lib-table` through `osl-kicad`'s `KicadSymbolLibraryIndex`, so search and symbol metadata stay in the Rust KiCad layer instead of being reparsed by the GUI; selecting a symbol shows a real canvas preview built from `KicadCanvasScene::from_symbol_definition`, unit/body-style/pin-alternate controls come from the same indexed KiCad metadata, and pressing `Place` starts a canvas-click placement flow with optional repeat placement and mouse-position preview on the schematic canvas. Placement resolves the `.kicad_sym` definition plus same-library inheritance context through the library adapter and applies `KicadSchematicEdit::PlaceSymbol` through the document adapter, so derived symbols embed their parent definitions while preserving KiCad `(extends ...)` semantics and selected unit/body style / pin alternates. After a GUI edit, selection is refreshed through the same Rust-native scene and hit-test path, so the GUI does not need to know KiCad file layout or geometry internals. The crate is split into app state/event routing, document adapter, library adapter, viewport transforms, panel composition, selection property editing, schematic tool routing/state/preview, symbol browser / preview, placement config/state, and canvas drawing modules; `crates/osl-app/README.md` records those boundaries for future GUI work. SVG rendering remains a headless regression baseline rather than the GUI rendering path.
...
`osl kicad-select <file.kicad_sch> <x,y>` runs the same Rust-native canvas scene through hit-testing and returns stable JSON for GUI selection. Hits include kind, KiCad UUID, label, bounds, and area, sorted from smaller bounds to larger bounds with deterministic tie-breakers. Symbols, wire, bus, bus-entry, sheet boxes, sheet pins, junctions, no-connect markers, rule areas, text boxes, tables/table cells, directive labels, and schematic drawing primitives use geometry-aware hit-testing after a bounds prefilter: symbols hit their body graphics and pin bodies, line-like objects use segment distance, Bezier curves use sampled cubic segments, KiCad three-point arcs use sampled circular arcs, hollow rectangles/circles/polygons hit their outlines, filled rectangles/circles/polygons and junctions hit their interior, sheet boxes hit their actual rectangles, sheet pins and directive labels hit their KiCad short lead segments, no-connect markers hit their X strokes, rotated text boxes and table cells hit their rotated rectangles, and labels/text/directive-label text use estimated text boxes from text effects and justification. Other selectable objects currently use object-level bounds. This is the command-routing bridge for future select/move/delete workflows on `.kicad_sch` assets while exact glyph metrics and pin label bounds are added later.
...
`kicad_demo_smoke` is an external interoperability smoke test. It reads representative KiCad demo projects from `kicad-source-mirror-master/demos` when the local KiCad source mirror is present, or from `NEKOSPICE_KICAD_DEMOS` when set. The test opens KiCad `.kicad_pro`, `.kicad_sch`, and local `sym-lib-table` assets, builds Rust-native canvas scenes, and checks that simulation, hierarchy, bus, and large-graphic demo features survive the KiCad-compatible IR. `kicad_demo_import_smoke` uses the same external demo root for `osl-netlist` and verifies real KiCad simulation projects can resolve from `.kicad_pro` to root schematic, import into a SPICE deck, surface stable diagnostics such as `kicad-missing-ground`, and generate normalized project artifacts. If no demo directory is available, both tests skip without adding KiCad sources or demo files to Git.
