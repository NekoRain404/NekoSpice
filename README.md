...
`osl kicad-select <file.kicad_sch> <x,y>` runs the same Rust-native canvas scene through object-level bounds hit-testing and returns stable JSON for GUI selection. Hits include kind, KiCad UUID, label, bounds, and area, sorted from smaller bounds to larger bounds with deterministic tie-breakers. This is the command-routing bridge for future select/move/delete workflows on `.kicad_sch` assets while more exact shape hit-testing is added later.
