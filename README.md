...
`osl kicad-inspect <file.kicad_sch> --canvas` emits full canvas scene JSON for downstream GUI work, including symbols, transformed symbol graphics, pins with alternate/text-effect metadata, sheet boxes and sheet pins, schematic graphics, embedded images, tables/table cells, rule areas, wires, buses, bus entries, directive labels with properties, labels, text items, text boxes, junctions, no-connect markers, KiCad stroke/fill/text effects, margins, counts, and bounds. `KicadCanvasScene::to_summary_json()` remains available for compact counts.
