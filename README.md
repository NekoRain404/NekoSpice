...
`osl kicad-select <file.kicad_sch> <x,y>` runs the same Rust-native canvas scene through hit-testing and returns stable JSON for GUI selection. Hits include kind, KiCad UUID, label, bounds, and area, sorted from smaller bounds to larger bounds with deterministic tie-breakers. Wire, bus, bus-entry, rule areas, and schematic drawing primitives use geometry-aware hit-testing after a bounds prefilter: line-like objects use segment distance, Bezier curves use sampled cubic segments, hollow rectangles/circles/polygons hit their outlines, and filled rectangles/circles/polygons hit their interior. Other selectable objects currently use object-level bounds. This is the command-routing bridge for future select/move/delete workflows on `.kicad_sch` assets while more exact shape hit-testing is added for the remaining object types.
...
`osl kicad-edit ... delete-item:<uuid>` removes top-level schematic canvas items by KiCad UUID, including symbols, wires, buses, bus entries, junctions, no-connect markers, labels, directive labels, text, sheets, drawing graphics, rule areas, images, tables, and groups, giving the future GUI a stable Rust-native deletion path without touching shared library symbol definitions.

`osl kicad-edit ... move-item:<uuid>:<dx,dy>` translates top-level schematic geometry by KiCad UUID, including placed symbols with their properties, wires, buses, bus entries, junctions, no-connect markers, labels, directive labels and their properties, text, sheets with sheet properties and pins, drawing graphics, rule areas, images, and table cells. Group records remain metadata-only; move the referenced member items for a geometry change.
