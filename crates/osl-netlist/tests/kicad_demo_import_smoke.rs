use osl_netlist::{ImportSeverity, NetlistFlavor, read_import_input};
use std::env;
...
const DEMO_IMPORT_CASES: &[DemoImportCase] = &[
    DemoImportCase {
        ...
        project: "simulation/rectifier/rectifier.kicad_pro",
        schematic: "simulation/rectifier/rectifier.kicad_sch",
        min_components: 5,
        min_symbols: 5,
        min_directives: 3,
    ...
    DemoImportCase {
        ...
        project: "simulation/sallen_key/sallen_key.kicad_pro",
        schematic: "simulation/sallen_key/sallen_key.kicad_sch",
        min_components: 7,
        min_symbols: 7,
        min_directives: 3,
    ...
    DemoImportCase {
        ...
        project: "simulation/subsheets/mainsheet.kicad_pro",
        schematic: "simulation/subsheets/mainsheet.kicad_sch",
        min_components: 13,
        min_symbols: 13,
        min_directives: 2,
        expected_directive: ".tran",
    ...
    DemoImportCase {
        ...
        project: "simulation/v_i_sources/v_i_sources.kicad_pro",
        schematic: "simulation/v_i_sources/v_i_sources.kicad_sch",
        min_components: 30,
        min_symbols: 30,
        min_directives: 2,
        expected_directive: ".tran",
...
#[test]
fn imports_representative_kicad_simulation_demos_from_reference_source() {
    let Some(demo_root) = kicad_demo_root() else {
        eprintln!(
            "skipping KiCad demo import smoke test: set NEKOSPICE_KICAD_DEMOS to a KiCad demos directory"
        );
        return;
    };
...
fn smoke_demo_import_case(demo_root: &Path, case: &DemoImportCase) {
    ...
    assert_eq!(
        report.flavor,
        NetlistFlavor::KiCad,
        "{} should be classified as a KiCad import",
        case.name
    );
    assert!(
    ...
    assert!(
        report.diagnostics.iter().any(|diagnostic| {
            diagnostic.severity == ImportSeverity::Error
                && diagnostic.code == "kicad-missing-ground"
        }),
        "{} should surface KiCad missing-ground diagnostics explicitly",
        case.name
    );
...
fn assert_normalized_project(
    ...
    assert!(
        netlist.starts_with("* Imported from KiCad schematic:"),
    ...
    assert!(
        manifest_json.contains("\"flavor\": \"kicad\""),
        "{} normalized manifest should record the KiCad flavor",
        case.name
    );
    assert!(
...
fn kicad_demo_root() -> Option<PathBuf> {
    if let Some(path) = env::var_os("NEKOSPICE_KICAD_DEMOS").map(PathBuf::from) {
...
fn workspace_relative_demo_root() -> Option<PathBuf> {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .map(|workspace| workspace.join("kicad-source-mirror-master/demos"))
}
...
fn current_dir_demo_roots() -> Vec<PathBuf> {
    env::current_dir()
        ...
        .flat_map(|cwd| {
            cwd.ancestors()
                .map(|path| path.join("kicad-source-mirror-master/demos"))
                .collect::<Vec<_>>()
        })
        .collect()
}
...
fn valid_demo_root(path: PathBuf) -> Option<PathBuf> {
    path.join("simulation/rectifier/rectifier.kicad_sch")
        .is_file()
        .then_some(path)
}
