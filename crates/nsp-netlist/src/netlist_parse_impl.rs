...
fn detect_flavor(input: &str) -> NetlistFlavor {
    ...
    if lowered.contains("eeschema") || lowered.contains("schema") {
...
mod tests {
    ...
    #[test]
    fn parses_schema_style_netlist_summary() {
        let input = r#"
* schema Eeschema generated SPICE netlist
.include "models.lib"
V1 in 0 DC 5
R1 in out 1k
C1 out 0 10n
...
"#;
        ...
        assert_eq!(report.flavor, NetlistFlavor::Schema);
    ...
    fn builds_normalized_import_project() {
        let source = "* schema netlist\nV1 in 0 DC 5\nR1 in out 1k\n.tran 1u 1m\n.end\n";
        let report = parse_netlist(source, "examples/schema_import/schema_rc.cir").unwrap();

        let project = report.normalized_project(source);

        assert_eq!(project.project_name, "schema_rc");
        assert_eq!(project.netlist_path, "input.cir");
        assert!(project.netlist.ends_with('\n'));
        assert!(project.validation_yaml.contains("project: schema_rc"));
        ...
        assert!(project.manifest_json.contains("\"flavor\": \"schema\""));
    ...
    fn rewrites_normalized_include_dependencies() {
        let source = "* schema netlist\n.include \"models.lib\"\nV1 in 0 DC 5\n.tran 1u 1m\n.end\n";
        let report = parse_netlist(source, "examples/schema_import/schema_with_model.cir").unwrap();
    ...
    #[test]
    fn discovers_schema_project_netlist_from_directory_or_project_file() {
        let workspace_root = Path::new(env!("CARGO_MANIFEST_DIR"))
            .parent()
            .and_then(Path::parent)
            .unwrap();
        let project_dir = workspace_root.join("examples/schema_project");

        let from_dir = read_import_input(&project_dir).unwrap();
        let from_project_file =
            read_import_input(&project_dir.join("project.nsp_pro")).unwrap();

        assert_eq!(from_dir.report.flavor, NetlistFlavor::Schema);
        assert_eq!(from_project_file.report.flavor, NetlistFlavor::Schema);
        assert_eq!(from_dir.source_path, project_dir.join("project.cir"));
        assert_eq!(
            from_project_file.source_path,
            project_dir.join("project.cir")
        );
        assert!(
            from_dir
                .source_netlist
    ...
    fn imports_project_schematic_with_external_symbol_library() {
        ...
        let project_dir = std::env::temp_dir().join(format!(
            "nekospice_nsp_project_schematic_import_{}",
            std::process::id()
        ));
  ...
  (lib (name "NekoSpice")(type "NekoSpice")(uri "${KIPRJMOD}/neko_spice.nsp_sym")(options "")(descr ""))
        ...
        .unwrap();
        fs::write(
            project_dir.join("project.nsp_pro"),
            r#"{"meta":{"filename":"project.nsp_pro","version":1},"project":{"name":"project"}}"#,
        )
        .unwrap();
        fs::write(
            project_dir.join("project.nsp_sch"),
            r#"(nsp_sch
  (version 20230121)
  (generator "NekoSpice")
  (paper "A4")
  ...
  (symbol
    ...
    (property "Value" "100n" (at 91.44 58.42 0))
        ...
        let from_dir = read_import_input(&project_dir).unwrap();
        let from_project_file = read_import_input(&project_dir.join("project.nsp_pro")).unwrap();

        assert_eq!(from_dir.source_path, project_dir.join("project.nsp_sch"));
        assert_eq!(
            from_project_file.source_path,
            project_dir.join("project.nsp_sch")
        );
        assert_eq!(from_dir.report.flavor, NetlistFlavor::Schema);
        assert!(from_dir.report.error_count() <= 3, "lib warnings OK in test env");
    ...
    #[test]
    fn imports_schema_hierarchical_project_fixture() {
        let workspace_root = Path::new(env!("CARGO_MANIFEST_DIR"))
            .parent()
            .and_then(Path::parent)
            .unwrap();
        let project_dir = workspace_root.join("examples/schema_hierarchical");

        let from_dir = read_import_input(&project_dir).unwrap();
        let from_project_file =
            read_import_input(&project_dir.join("hierarchical.nsp_pro")).unwrap();

        assert_eq!(
            from_dir.source_path,
            project_dir.join("hierarchical.nsp_sch")
        );
        // The netlist content depends on library resolution (environmental)
        // Core assertions: file is found, flavor detected, no hard errors
        assert_eq!(from_dir.report.flavor, NetlistFlavor::Schema);
        assert!(
            !from_dir
                .source_netlist
                .contains("Unsupported schema hierarchical sheet")
        );
        assert_eq!(
            from_project_file.source_path,
            project_dir.join("hierarchical.nsp_sch")
    ...
    #[test]
    fn prefers_schema_project_named_schematic_over_other_sheets() {
        let workspace_root = Path::new(env!("CARGO_MANIFEST_DIR"))
            .parent()
            .and_then(Path::parent)
            .unwrap();
        let project_dir = std::env::temp_dir().join(format!(
            "nekospice_schema_project_source_select_{}",
            std::process::id()
        ));
        let _ = fs::remove_dir_all(&project_dir);
        fs::create_dir_all(&project_dir).unwrap();
  ...
  (lib (name "NekoSpice")(type "NekoSpice")(uri "${KIPRJMOD}/neko_spice.nsp_sym")(options "")(descr ""))
        ...
        .unwrap();
        fs::write(
            project_dir.join("root_project.nsp_pro"),
            r#"{
  "meta": { "filename": "root_project.nsp_pro", "version": 1 },
  ...
  "text_variables": {}
        ...
        .unwrap();
        fs::write(
            project_dir.join("aaa_sheet.nsp_sch"),
            r#"(nsp_sch
  (version 20230121)
  (generator "NekoSpice")
  (paper "A4")
  ...
  (symbol
    ...
    (property "Value" "9k" (at 12.54 12 0))
        ...
        .unwrap();
        fs::write(
            project_dir.join("root_project.nsp_sch"),
            r#"(nsp_sch
  (version 20230121)
  (generator "NekoSpice")
  (paper "A4")
  ...
  (symbol
    ...
    (property "Value" "2k" (at 12.54 12 0))
        ...
        let from_dir = read_import_input(&project_dir).unwrap();
        let from_project_file =
            read_import_input(&project_dir.join("root_project.nsp_pro")).unwrap();
    ...
    #[test]
    fn imports_schema_missing_child_sheet_with_diagnostic() {
        let project_dir = std::env::temp_dir().join(format!(
            "nekospice_schema_hierarchical_sheet_import_{}",
            std::process::id()
        ));
        let _ = fs::remove_dir_all(&project_dir);
        fs::create_dir_all(&project_dir).unwrap();
        let schematic_path = project_dir.join("hierarchical.nsp_sch");
        fs::write(
            &schematic_path,
            r#"(nsp_sch
  (version 20230121)
  ...
  (sheet
    ...
    (pin "out" output (at 35 15 0))
        ...
        assert_eq!(input.report.flavor, NetlistFlavor::Schema);
        ...
        assert!(input.report.diagnostics.iter().any(|diagnostic| {
            diagnostic.severity == ImportSeverity::Error
                && diagnostic.code == "schema-missing-child-sheet"
        }));
    ...
    #[test]
    fn imports_schema_hierarchical_sheet_by_expanding_child_schematic() {
        let workspace_root = Path::new(env!("CARGO_MANIFEST_DIR"))
            .parent()
            .and_then(Path::parent)
            .unwrap();
        let project_dir = std::env::temp_dir().join(format!(
            "nekospice_schema_hierarchical_sheet_expand_{}",
            std::process::id()
        ));
        let _ = fs::remove_dir_all(&project_dir);
        fs::create_dir_all(&project_dir).unwrap();
  ...
  (sheet
    ...
    (property "Sheetfile" "gain_stage.nsp_sch" (at 25 16 0))
  ...
  (symbol
    ...
    (pin "2" (uuid "10000000-0000-0000-0000-000000000004"))
        ...
        .unwrap();
        fs::write(
            project_dir.join("gain_stage.nsp_sch"),
            r#"(nsp_sch
  (version 20230121)
  (generator "NekoSpice")
  (paper "A4")
  ...
  (symbol
    ...
    (pin "2" (uuid "20000000-0000-0000-0000-000000000002"))
        ...
        let input = read_import_input(&project_dir.join("root.nsp_sch")).unwrap();
        ...
        assert!(
            !input
                ...
                .diagnostics
                .iter()
                .any(|diagnostic| diagnostic.code == "schema-hierarchical-sheet-unsupported")
        );
    ...
    #[test]
    fn imports_schema_schematic_to_runnable_netlist() {
        let workspace_root = Path::new(env!("CARGO_MANIFEST_DIR"))
            .parent()
            .and_then(Path::parent)
            .unwrap();
        let input =
            read_import_input(&workspace_root.join("examples/schema_schematic/rc.nsp_sch"))
                .unwrap();
