//! Netlist parsing and conversion — Schema, LTspice, and SPICE format support.

mod ltspice_import;
mod schema_import;

use ltspice_import::{import_ltspice_asc, is_ltspice_schematic};
use nsp_core::{OslResult, html_escape, json_escape, read_text};
use nsp_schema::read_schematic_hierarchy_netlist;
use schema_import::{is_schema_schematic, resolve_import_source_path, schema_diagnostic_to_import};
...
pub fn read_import_input(path: &Path) -> OslResult<ImportInput> {
    ...
    if is_schema_schematic(&path) {
        let exported = read_schematic_hierarchy_netlist(&path)?;
        let mut report = parse_netlist(&exported.netlist, &source)?;
        report.flavor = NetlistFlavor::Schema;
        report
            .diagnostics
            .extend(exported.diagnostics.iter().map(schema_diagnostic_to_import));
        Ok(ImportInput {
