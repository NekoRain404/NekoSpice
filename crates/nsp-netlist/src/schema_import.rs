...
pub(crate) fn is_schema_schematic(path: &Path) -> bool {
    path.extension()
        ...
        .is_some_and(|extension| extension.eq_ignore_ascii_case("nsp_sch"))
}
...
pub(crate) fn schema_diagnostic_to_import(diagnostic: &NspSchematicDiagnostic) -> ImportDiagnostic {
    ...
    let suggestion = if diagnostic.code == "hierarchical-sheet-unsupported" {
        ...
        "Run `nk schema-check <file.nsp_sch>` and fix the schematic before relying on the generated SPICE netlist.".to_string()
    };
...
fn is_schema_project_file(path: &Path) -> bool {
    path.extension()
        .and_then(|extension| extension.to_str())
        .is_some_and(|extension| extension.eq_ignore_ascii_case("nsp_pro"))
}
...
fn discover_project_schematic(
    ...
    candidates.into_iter().next().ok_or_else(|| {
        OslError::InvalidInput(format!(
            "{} does not contain an importable schematic (.nsp_sch)",
            project_dir.display()
        ))
    })
}
