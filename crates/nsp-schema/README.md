...
`nsp-schema` is the Rust-native format-compatible schematic, project, symbol
library, canvas-scene, edit, and netlist foundation for NekoSpice.

## Boundaries

- `lib.rs` currently exposes the public schematic API and still owns top-level
  root API assembly.
...
- `schematic_io.rs` owns top-level `.nsp_sch` read/write entry points, root
...
- `simulation.rs` owns structured SPICE/simulation directive discovery,
  ...
  schematic edit helper for format-compatible simulation text items.
- `spice_export.rs` owns schematic-to-SPICE export, hierarchy expansion,
  ...
  scoping.
- `style.rs` owns reusable stroke/fill/color/text-effects/margins IR,
...
- `coordinates.rs` owns reusable point/size/at IR, coordinate parse
...
- `project.rs` owns `.nsp_pro` project JSON parsing, project summary DTOs, and
  schematic stem candidate discovery used by Schema project import paths.
- `symbol_library.rs` owns `.nsp_sym` library containers, `sym-lib-table`
