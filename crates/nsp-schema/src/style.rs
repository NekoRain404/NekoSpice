//! schema visual style — fill, stroke, margins, colors, and text effects.
...
use crate::{NspSize, parse_size};

#[derive(Debug, Clone, PartialEq)]
pub struct NspStroke {
...
#[derive(Debug, Clone, PartialEq)]
pub struct NspTextEffects {
    pub font_size: Option<NspSize>,
    pub font_thickness: Option<f64>,
    pub font_bold: Option<bool>,
    pub font_italic: Option<bool>,
    pub font_color: Option<NspColor>,
...
/// default schema text effects。
pub(crate) fn default_schema_text_effects() -> NspTextEffects {
    NspTextEffects {
        font_size: Some(NspSize {
            width: 1.27,
            height: 1.27,
        }),
        font_thickness: None,
...
/// parse text effects。
pub(crate) fn parse_text_effects(node: &Sexp) -> NspTextEffects {
...
fn parse_effect_bool(items: &[Sexp], name: &str) -> Option<bool> {
    child(items, name)
        .and_then(|node| list_value(node, 1).and_then(parse_bool_value))
...
/// parse color。
pub(crate) fn parse_color(node: &Sexp) -> Option<NspColor> {
    let items = list_items(node);
    Some(NspColor {
...
/// parse stroke。
pub(crate) fn parse_stroke(node: &Sexp) -> NspStroke {
    let items = list_items(node);
    NspStroke {
...
/// parse fill。
pub(crate) fn parse_fill(node: &Sexp) -> NspFill {
    let items = list_items(node);
    NspFill {
...
/// parse margins。
pub(crate) fn parse_margins(node: &Sexp) -> Option<NspMargins> {
    let items = list_items(node);
    Some(NspMargins {
...
pub(crate) fn write_inline_text_effects(output: &mut String, effects: Option<&NspTextEffects>) {
    match effects {
...
pub(crate) fn write_inline_optional_fill(output: &mut String, fill: Option<&NspFill>) {
...
/// write inline fill。
pub(crate) fn write_inline_fill(output: &mut String, fill: Option<&NspFill>) {
    match fill {
        Some(fill) => {
...
/// schema margins value。
pub(crate) fn schema_margins_value(margins: NspMargins) -> serde_json::Value {
    serde_json::json!({
        "left": margins.left,
        "top": margins.top,
        "right": margins.right,
...
/// schema color value。
pub(crate) fn schema_color_value(color: NspColor) -> serde_json::Value {
    serde_json::json!({
        "red": color.red,
        "green": color.green,
        "blue": color.blue,
...
/// schema stroke value。
pub(crate) fn schema_stroke_value(stroke: &NspStroke) -> serde_json::Value {
    serde_json::json!({
        "width": stroke.width,
        "type": stroke.stroke_type,
        "color": stroke.color.map(schema_color_value),
    })
}

/// schema fill value。
pub(crate) fn schema_fill_value(fill: &NspFill) -> serde_json::Value {
    serde_json::json!({
        "type": fill.fill_type,
        "color": fill.color.map(schema_color_value),
    })
}

/// schema text effects value。
pub(crate) fn schema_text_effects_value(effects: &NspTextEffects) -> serde_json::Value {
    serde_json::json!({
        "font_size": effects.font_size.map(crate::schema_size_value),
        "font_thickness": effects.font_thickness,
        "font_bold": effects.font_bold,
        "font_italic": effects.font_italic,
        "font_color": effects.font_color.map(schema_color_value),
