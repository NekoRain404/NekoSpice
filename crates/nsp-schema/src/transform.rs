...
/// transform local at。
pub(crate) fn transform_local_at(local_at: NspAt, symbol_at: NspAt, mirror: Option<&str>) -> NspAt {
...
/// rotate point。
pub(crate) fn rotate_point(point: NspPoint, rotation: f64) -> NspPoint {
...
fn mirror_point(point: NspPoint, mirror: Option<&str>) -> NspPoint {
...
pub fn normalize_symbol_mirror(value: &str) -> OslResult<Option<String>> {
    ...
    symbol_mirror_from_axes(axes).map(Some).ok_or_else(|| {
        OslError::InvalidInput("schema symbol mirror must be x, y, xy, or none".to_string())
    })
}
...
fn insert_mirror_axis<'a>(axes: &mut BTreeSet<&'a str>, axis: &'a str) -> OslResult<()> {
    match axis {
        ...
        _ => Err(OslError::InvalidInput(format!(
            "unsupported schema symbol mirror axis '{axis}'"
        ))),
    }
}
