//! Symbol library index -- reads schematic symbol library files (compatible with .nsp_sym format) and provides lookup by name.

use crate::json::schema_bounding_box_value;
use crate::util::{case_insensitive_contains, resolve_uri, schema_wildcard_match};
use crate::{
    NspBoundingBox, NspDiagnosticSeverity, NspPinAlternate, NspResolvedSymbolDef,
    NspSymbolLibraryTable, read_symbol_library, resolve_symbol_definition,
    schema_pin_alternate_value,
};
...
pub struct NspSymbolLibraryIndex {
    pub source: String,
    pub libraries: Vec<NspIndexedLibrary>,
    pub symbols: Vec<NspIndexedSymbol>,
    pub diagnostics: Vec<NspLibraryDiagnostic>,
}

impl NspSymbolLibraryIndex {
    /// from table。
    pub fn from_table(table: NspSymbolLibraryTable, base_dir: &Path) -> Self {
        ...
        for row in table.libraries {
            ...
            if !row.library_type.eq_ignore_ascii_case("NekoSpice") {
                diagnostics.push(NspLibraryDiagnostic {
                    library: row.name.clone(),
            ...
            match read_symbol_library(&resolved_path) {
                Ok(library) => {
                    ...
                    libraries.push(NspIndexedLibrary {
                        name: row.name,
                        source: resolved_path.display().to_string(),
                        symbol_count,
                    });
    ...
    /// symbol。
    pub fn symbol(&self, lib_id: &str) -> Option<&NspIndexedSymbol> {
        self.symbols.iter().find(|symbol| symbol.id == lib_id)
    }

    /// query。
    pub fn query(&self, query: &NspSymbolLibraryIndexQuery) -> Self {
        let symbols = self
            .symbols
            .iter()
    ...
    pub fn to_json(&self) -> String {
        serde_json::to_string_pretty(&self.to_json_value())
            .expect("schema symbol library index JSON should serialize")
    }
    ...
    fn to_json_value(&self) -> serde_json::Value {
        serde_json::json!({
            ...
            "libraries": self.libraries.iter().map(NspIndexedLibrary::to_json_value).collect::<Vec<_>>(),
            "symbols": self.symbols.iter().map(NspIndexedSymbol::to_json_value).collect::<Vec<_>>(),
            "diagnostics": self.diagnostics.iter().map(NspLibraryDiagnostic::to_json_value).collect::<Vec<_>>(),
        })
    }
...
pub struct NspSymbolLibraryIndexQuery {
    pub text: Option<String>,
    pub library: Option<String>,
    pub footprint: Option<String>,
}

impl NspSymbolLibraryIndexQuery {
    /// is empty。
    ...
    fn matches(&self, symbol: &NspIndexedSymbol) -> bool {
    ...
    fn matches_text(&self, symbol: &NspIndexedSymbol) -> bool {
    ...
    fn matches_footprint(&self, symbol: &NspIndexedSymbol) -> bool {
        ...
        symbol
            .footprint_filters
            .iter()
            .any(|filter| schema_wildcard_match(filter, footprint))
    }
}

#[derive(Debug, Clone, PartialEq)]
pub struct NspIndexedLibrary {
    pub name: String,
    pub source: String,
    pub symbol_count: usize,
}

impl NspIndexedLibrary {
    fn to_json_value(&self) -> serde_json::Value {
        serde_json::json!({
            "name": self.name,
            "source": self.source,
...
pub struct NspIndexedSymbol {
    ...
    pub units: Vec<NspIndexedSymbolUnit>,
    pub body_styles: Vec<NspIndexedSymbolBodyStyle>,
    pub pins: Vec<NspIndexedSymbolPin>,
    pub extends: Option<String>,
    pub power: Option<String>,
    pub bounding_box: Option<NspBoundingBox>,
}

impl NspIndexedSymbol {
    fn to_json_value(&self) -> serde_json::Value {
        serde_json::json!({
            "id": self.id,
            ...
            "unit_count": self.unit_count,
            "units": self.units.iter().map(NspIndexedSymbolUnit::to_json_value).collect::<Vec<_>>(),
            "body_styles": self.body_styles.iter().map(NspIndexedSymbolBodyStyle::to_json_value).collect::<Vec<_>>(),
            "pins": self.pins.iter().map(NspIndexedSymbolPin::to_json_value).collect::<Vec<_>>(),
            "extends": self.extends,
            "power": self.power,
            "bounding_box": self.bounding_box.map(schema_bounding_box_value),
        })
    }
}

#[derive(Debug, Clone, PartialEq, Eq)]
pub struct NspIndexedSymbolUnit {
    pub unit: u32,
    pub name: Option<String>,
}

impl NspIndexedSymbolUnit {
    fn to_json_value(&self) -> serde_json::Value {
        serde_json::json!({
            "unit": self.unit,
            "name": self.name,
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct NspIndexedSymbolBodyStyle {
    pub body_style: u32,
    pub name: Option<String>,
}

impl NspIndexedSymbolBodyStyle {
    fn to_json_value(&self) -> serde_json::Value {
        serde_json::json!({
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct NspIndexedSymbolPin {
    pub number: String,
    pub name: String,
...
#[derive(Debug, Clone, PartialEq)]
pub struct NspLibraryDiagnostic {
    pub library: String,
    pub severity: NspDiagnosticSeverity,
    pub message: String,
}

impl NspLibraryDiagnostic {
    fn to_json_value(&self) -> serde_json::Value {
        serde_json::json!({
