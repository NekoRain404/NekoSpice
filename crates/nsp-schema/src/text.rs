//! schema text elements — text items and text boxes.

use crate::coordinates::{NspAt, NspSize, parse_at, parse_size};
use crate::geometry::{NspBoundingBox, schema_rotated_rect_bounds};
...
use crate::style::{
    ...
    parse_text_effects, write_inline_fill, write_inline_stroke, write_inline_text_effects,
    write_text_effects_line,
};
use crate::util::{parse_bool_value, parse_optional_bool_child};

#[derive(Debug, Clone, PartialEq)]
pub struct NspTextItem {
    pub text: String,
    pub at: Option<NspAt>,
...
/// parse text item。
pub(crate) fn parse_text_item(node: &Sexp) -> Option<NspTextItem> {
...
/// parse text box。
pub(crate) fn parse_text_box(node: &Sexp) -> Option<NspTextBox> {
    let items = list_items(node);
    Some(NspTextBox {
        text: list_value(node, 1)?,
        at: child(items, "at").and_then(parse_at),
