//! schema schematic I/O - reading, writing, parsing, and serialization.
...
use crate::sexpr::{
    child, child_value, direct_children, expect_root_list_any, list_items, sexpr_atom_or_string,
    sexpr_string,
};
...
use crate::{NspLabelKind, NspSchematic, parse_sexpr};
use nsp_core::{OslResult, read_text, write_text};
use std::path::Path;

/// read schema schematic。
pub fn read_schematic(path: &Path) -> OslResult<NspSchematic> {
    let content = read_text(path)?;
    parse_schematic(&content, &path.display().to_string())
}

/// read schema schematic with libraries。
pub fn read_schematic_with_libraries(path: &Path) -> OslResult<NspSchematic> {
    let mut schematic = read_schematic(path)?;
...
pub fn parse_schematic(input: &str, source: &str) -> OslResult<NspSchematic> {
    let root = parse_sexpr(input)?;
    let root_list = expect_root_list_any(&root, &["nsp_sch", "kicad_sch"])?;
    ...
    Ok(NspSchematic {
        ...
        labels: direct_children(root_list, "label")
            .filter_map(|node| parse_label(node, NspLabelKind::Local))
            .chain(
                direct_children(root_list, "global_label")
                    .filter_map(|node| parse_label(node, NspLabelKind::Global)),
            )
            .chain(
                direct_children(root_list, "hierarchical_label")
                    .filter_map(|node| parse_label(node, NspLabelKind::Hierarchical)),
            )
            .collect(),
        ...
        symbol_instances: child(root_list, "symbol_instances")
            ...
            .unwrap_or_default(),
        embedded_fonts: child_value(root_list, "embedded_fonts").and_then(parse_bool_value),
    })
}
