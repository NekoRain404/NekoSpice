//! schema labels — local, global, hierarchical, and directive (netclass) labels.
...
use crate::style::{NspTextEffects, parse_text_effects, write_inline_text_effects};
...
/// parse label。
pub(crate) fn parse_label(node: &Sexp, kind: NspLabelKind) -> Option<NspLabel> {
...
pub(crate) fn parse_directive_label(node: &Sexp) -> Option<NspDirectiveLabel> {
    let items = list_items(node);
    Some(NspDirectiveLabel {
