...
use crate::coordinates::{
    NspPoint, NspSize, parse_at, parse_points, parse_size, write_points_sexpr,
};
use crate::sexpr::{
    Sexp, atom_text, child, child_value, format_number, head, list_items, list_value, sexpr_string,
    write_sexpr_inline,
};
use crate::style::{NspColor, NspStroke, parse_color, parse_stroke, write_inline_stroke};
...
#[derive(Debug, Clone, PartialEq)]
pub struct NspNetChain {
    pub name: String,
    pub from: Option<NspNetChainEndpoint>,
    pub to: Option<NspNetChainEndpoint>,
...
#[derive(Debug, Clone, PartialEq)]
pub struct NspNetChainEndpoint {
    pub reference: String,
    pub pin: String,
}

/// parse wire。
pub(crate) fn parse_wire(node: &Sexp) -> NspWire {
...
/// parse bus alias。
pub(crate) fn parse_bus_alias(node: &Sexp) -> Option<NspBusAlias> {
    let items = list_items(node);
    Some(NspBusAlias {
...
/// parse bus。
pub(crate) fn parse_bus(node: &Sexp) -> NspBus {
    let items = list_items(node);
    NspBus {
        points: child(items, "pts").map(parse_points).unwrap_or_default(),
        stroke: child(items, "stroke").map(parse_stroke),
...
/// parse bus entry。
pub(crate) fn parse_bus_entry(node: &Sexp) -> Option<NspBusEntry> {
    let items = list_items(node);
...
/// parse net chain。
pub(crate) fn parse_net_chain(node: &Sexp) -> Option<NspNetChain> {
    let items = list_items(node);
...
fn parse_net_chain_endpoint(node: &Sexp) -> Option<NspNetChainEndpoint> {
    Some(NspNetChainEndpoint {
