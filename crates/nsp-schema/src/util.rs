//! schema parsing utilities — boolean parsing and string helpers.

use crate::sexpr::{Sexp, child, list_value};
use std::env;
use std::path::{Path, PathBuf};

/// parse schema footprint filters。
pub(crate) fn parse_footprint_filters(value: &str) -> Vec<String> {
    value
        .split_whitespace()
        .map(unescape_schema_brace_string)
...
pub(crate) fn schema_wildcard_match(pattern: &str, value: &str) -> bool {
    wildcard_match(
...
fn unescape_schema_brace_string(value: &str) -> String {
    ...
    while let Some(character) = characters.next() {
        ...
        if terminated {
            match token.as_str() {
                ...
                _ => {
                    output.push('{');
                    output.push_str(&unescape_schema_brace_string(&token));
                    output.push('}');
                }
            }
        } else {
...
pub(crate) fn resolve_uri(uri: &str, base_dir: &Path) -> PathBuf {
    let base_dir = normalize_base_dir(base_dir);
    let expanded = expand_schema_uri(uri, &base_dir);
...
fn expand_schema_uri(uri: &str, base_dir: &Path) -> String {
...
/// parse schema bool value。
pub(crate) fn parse_bool_value(value: String) -> Option<bool> {
