...
pub struct NspSheetInstance {
    pub path: String,
    pub page: Option<String>,
}

#[derive(Debug, Clone, PartialEq)]
pub struct NspSymbolPathInstance {
    pub path: String,
    pub reference: Option<String>,
    pub unit: Option<u32>,
    pub value: Option<String>,
    pub footprint: Option<String>,
    pub variants: Vec<NspVariantInstance>,
}

#[derive(Debug, Clone, PartialEq)]
pub struct NspProjectInstance {
    pub name: String,
    pub paths: Vec<NspInstancePath>,
}

#[derive(Debug, Clone, PartialEq)]
pub struct NspInstancePath {
    pub path: String,
    pub page: Option<String>,
    pub reference: Option<String>,
    pub unit: Option<u32>,
...
#[derive(Debug, Clone, PartialEq)]
pub struct NspVariantInstance {
    pub name: Option<String>,
    pub dnp: Option<bool>,
}

/// parse sheet instances。
pub(crate) fn parse_sheet_instances(node: &Sexp) -> Vec<NspSheetInstance> {
    direct_children(list_items(node), "path")
        .filter_map(parse_sheet_instance)
        .collect()
}

fn parse_sheet_instance(node: &Sexp) -> Option<NspSheetInstance> {
    let items = list_items(node);
    Some(NspSheetInstance {
...
pub(crate) fn parse_symbol_path_instances(node: &Sexp) -> Vec<NspSymbolPathInstance> {
    direct_children(list_items(node), "path")
        .filter_map(parse_symbol_path_instance)
        .collect()
}

fn parse_symbol_path_instance(node: &Sexp) -> Option<NspSymbolPathInstance> {
    let path = parse_instance_path(node)?;
    Some(NspSymbolPathInstance {
        path: path.path,
...
pub(crate) fn parse_project_instances(node: &Sexp) -> Vec<NspProjectInstance> {
    direct_children(list_items(node), "project")
        .filter_map(parse_project_instance)
        .collect()
}

fn parse_project_instance(node: &Sexp) -> Option<NspProjectInstance> {
    let items = list_items(node);
    Some(NspProjectInstance {
...
fn parse_instance_path(node: &Sexp) -> Option<NspInstancePath> {
    let items = list_items(node);
    Some(NspInstancePath {
        path: list_value(node, 1)?,
        page: child_value(items, "page"),
...
fn parse_variant_instance(node: &Sexp) -> Option<NspVariantInstance> {
    let items = list_items(node);
    Some(NspVariantInstance {
...
pub(crate) fn write_symbol_path_instances_sexpr(
    output: &mut String,
    instances: &[NspSymbolPathInstance],
    indent: usize,
) {
    let pad = " ".repeat(indent);
...
fn write_instance_path_sexpr(output: &mut String, path: &NspInstancePath, indent: usize) {
    let pad = " ".repeat(indent);
...
fn write_variant_instance_sexpr(output: &mut String, variant: &NspVariantInstance, indent: usize) {
    let pad = " ".repeat(indent);
