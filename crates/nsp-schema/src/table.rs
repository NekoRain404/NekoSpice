//! schema table element parsing and serialization.

use crate::coordinates::{NspAt, NspSize, parse_at, parse_size};
use crate::geometry::{NspBoundingBox, NspBoundingBoxBuilder, schema_rotated_rect_bounds};
use crate::sexpr::{
    Sexp, atom_text, child, child_value, direct_children, format_number, list_items, list_value,
    sexpr_string,
};
use crate::style::{
    NspFill, NspMargins, NspStroke, NspTextEffects, parse_fill, parse_margins, parse_stroke,
    parse_text_effects, write_inline_fill, write_inline_optional_bool_sexpr, write_inline_stroke,
    write_text_effects_line,
};
...
pub struct NspTable {
    ...
    pub border: Option<NspTableBorder>,
    pub separators: Option<NspTableSeparators>,
    pub column_widths: Vec<f64>,
    pub row_heights: Vec<f64>,
    pub cells: Vec<NspTableCell>,
...
impl NspTableCell {
    /// bounding box。
    pub fn bounding_box(&self) -> Option<NspBoundingBox> {
        schema_rotated_rect_bounds(self.at?, self.size?)
    }
...
/// parse table。
pub(crate) fn parse_table(node: &Sexp) -> Option<NspTable> {
...
fn parse_table_cell(node: &Sexp) -> Option<NspTableCell> {
    let items = list_items(node);
...
fn parse_table_border(node: &Sexp) -> NspTableBorder {
    let items = list_items(node);
    NspTableBorder {
        external: child_value(items, "external").and_then(parse_bool_value),
        header: child_value(items, "header").and_then(parse_bool_value),
        stroke: child(items, "stroke").map(parse_stroke),
    }
}

fn parse_table_separators(node: &Sexp) -> NspTableSeparators {
    let items = list_items(node);
    NspTableSeparators {
        rows: child_value(items, "rows").and_then(parse_bool_value),
        cols: child_value(items, "cols").and_then(parse_bool_value),
        stroke: child(items, "stroke").map(parse_stroke),
    }
}

fn write_table_border_sexpr(output: &mut String, indent: usize, border: Option<&NspTableBorder>) {
    let pad = " ".repeat(indent);
...
fn write_table_separators_sexpr(
    output: &mut String,
    indent: usize,
    separators: Option<&NspTableSeparators>,
) {
    let pad = " ".repeat(indent);
