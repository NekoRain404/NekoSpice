//! Schematic edit operations — delete, move, rotate, place, wire, label, and more.

use crate::{
    NspAt, NspGraphic, NspLabelKind, NspPoint, NspProperty, NspSheet, NspSheetPin,
    NspSimulationDirectiveKind, NspSize, NspSymbolDef, NspTable, coordinate_key, format_number,
};
use nsp_core::{OslError, OslResult};
use std::collections::BTreeMap;

#[derive(Debug, Clone, PartialEq)]
pub enum NspSchematicEdit {
    MoveSymbol {
        reference: String,
        to: NspPoint,
        rotation: Option<f64>,
    },
    MoveItem {
        uuid: String,
        delta: NspPoint,
    },
    DeleteItem {
        uuid: String,
    },
    ConfigureSymbol {
        reference: String,
        unit: Option<u32>,
        body_style: Option<Option<u32>>,
        mirror: Option<Option<String>>,
        pin_alternates: Option<BTreeMap<String, String>>,
    },
    SetSymbolProperty {
        reference: String,
        name: String,
        value: String,
        at: Option<NspAt>,
    },
    PlaceSymbol {
        definition: Box<NspSymbolDef>,
        library_symbols: Vec<NspSymbolDef>,
        reference: String,
        value: String,
        at: NspAt,
        unit: Option<u32>,
        body_style: Option<u32>,
        pin_alternates: BTreeMap<String, String>,
        uuid: Option<String>,
    },
    AddWire {
        points: Vec<NspPoint>,
        uuid: Option<String>,
    },
    AddBus {
        points: Vec<NspPoint>,
        uuid: Option<String>,
    },
    AddBusEntry {
        at: NspPoint,
        size: NspSize,
        uuid: Option<String>,
    },
    AddJunction {
        at: NspPoint,
        uuid: Option<String>,
    },
    AddNoConnect {
        at: NspPoint,
        uuid: Option<String>,
    },
    AddLabel {
        text: String,
        kind: NspLabelKind,
        at: NspAt,
        uuid: Option<String>,
    },
    AddSheet {
        name: String,
        file: String,
        at: NspAt,
        size: NspSize,
        pins: Vec<NspSheetPin>,
        uuid: Option<String>,
    },
    AddText {
        text: String,
        at: NspAt,
        uuid: Option<String>,
    },
    /// Rotate an item by the given angle (degrees, typically 90).
    RotateItem {
        uuid: String,
        angle: f64,
    },
    SetSimulationDirective {
        kind: NspSimulationDirectiveKind,
        body: String,
        at: Option<NspAt>,
        uuid: Option<String>,
    },
}

#[derive(Debug, Clone, PartialEq)]
pub struct NspEditSummary {
    pub operation: String,
    pub target: String,
}

#[derive(Debug, Clone, PartialEq)]
pub struct NspSymbolPlacement {
    pub definition: NspSymbolDef,
    pub library_symbols: Vec<NspSymbolDef>,
    pub reference: String,
    pub value: String,
    pub at: NspAt,
    pub unit: Option<u32>,
    pub body_style: Option<u32>,
    pub pin_alternates: BTreeMap<String, String>,
    pub uuid: Option<String>,
}

/// remove by uuid。
pub(crate) fn remove_by_uuid<T>(
    items: &mut Vec<T>,
    uuid: &str,
    item_uuid: impl Fn(&T) -> Option<&str>,
) -> bool {
    if let Some(index) = items.iter().position(|item| item_uuid(item) == Some(uuid)) {
        items.remove(index);
        true
    } else {
        false
    }
}

/// remove table cell by uuid。
pub(crate) fn remove_table_cell_by_uuid(tables: &mut [NspTable], uuid: &str) -> bool {
    for table in tables {
        if remove_by_uuid(&mut table.cells, uuid, |cell| cell.uuid.as_deref()) {
            return true;
        }
    }
    false
}

/// remove sheet pin by uuid。
pub(crate) fn remove_sheet_pin_by_uuid(sheets: &mut [NspSheet], uuid: &str) -> bool {
    for sheet in sheets {
        if remove_by_uuid(&mut sheet.pins, uuid, |pin| pin.uuid.as_deref()) {
            return true;
        }
    }
    false
}

/// move table cell by uuid。
pub(crate) fn move_table_cell_by_uuid(
    tables: &mut [NspTable],
    uuid: &str,
    delta: NspPoint,
) -> bool {
    for table in tables {
        if let Some(cell) = table
            .cells
            .iter_mut()
            .find(|cell| cell.uuid.as_deref() == Some(uuid))
        {
            translate_optional_at(&mut cell.at, delta);
            return true;
        }
    }
    false
}

/// move sheet pin by uuid。
pub(crate) fn move_sheet_pin_by_uuid(sheets: &mut [NspSheet], uuid: &str, delta: NspPoint) -> bool {
    for sheet in sheets {
        if let Some(pin) = sheet
            .pins
            .iter_mut()
            .find(|pin| pin.uuid.as_deref() == Some(uuid))
        {
            translate_optional_at(&mut pin.at, delta);
            return true;
        }
    }
    false
}

/// translate point。
pub(crate) fn translate_point(point: &mut NspPoint, delta: NspPoint) {
    point.x += delta.x;
    point.y += delta.y;
}

/// translate optional point。
pub(crate) fn translate_optional_point(point: &mut Option<NspPoint>, delta: NspPoint) {
    if let Some(point) = point {
        translate_point(point, delta);
    }
}

/// translate points。
pub(crate) fn translate_points(points: &mut [NspPoint], delta: NspPoint) {
    for point in points {
        translate_point(point, delta);
    }
}

/// translate at。
pub(crate) fn translate_at(at: &mut NspAt, delta: NspPoint) {
    at.x += delta.x;
    at.y += delta.y;
}

/// translate optional at。
pub(crate) fn translate_optional_at(at: &mut Option<NspAt>, delta: NspPoint) {
    if let Some(at) = at {
        translate_at(at, delta);
    }
}

/// translate properties。
pub(crate) fn translate_properties(properties: &mut [NspProperty], delta: NspPoint) {
    for property in properties {
        translate_optional_at(&mut property.at, delta);
    }
}

/// translate graphic。
pub(crate) fn translate_graphic(graphic: &mut NspGraphic, delta: NspPoint) {
    match graphic {
        NspGraphic::Polyline { points } | NspGraphic::Bezier { points } => {
            translate_points(points, delta);
        }
        NspGraphic::Rectangle { start, end } => {
            translate_point(start, delta);
            translate_point(end, delta);
        }
        NspGraphic::Circle { center, .. } => translate_point(center, delta),
        NspGraphic::Arc { start, mid, end } => {
            translate_point(start, delta);
            translate_optional_point(mid, delta);
            translate_point(end, delta);
        }
        NspGraphic::Text { at, .. } => translate_optional_at(at, delta),
    }
}

/// move summary。
pub(crate) fn move_summary(kind: &str, uuid: &str) -> NspEditSummary {
    NspEditSummary {
        operation: format!("move-{kind}"),
        target: uuid.to_string(),
    }
}

/// delete summary。
pub(crate) fn delete_summary(kind: &str, uuid: &str) -> NspEditSummary {
    NspEditSummary {
        operation: format!("delete-{kind}"),
        target: uuid.to_string(),
    }
}

/// validate point。
pub(crate) fn validate_point(point: NspPoint, context: &str) -> OslResult<()> {
    if point.x.is_finite() && point.y.is_finite() {
        Ok(())
    } else {
        Err(OslError::InvalidInput(format!(
            "{context} coordinates must be finite"
        )))
    }
}

/// validate at。
pub(crate) fn validate_at(at: NspAt, context: &str) -> OslResult<()> {
    validate_point(NspPoint { x: at.x, y: at.y }, context)?;
    if at.rotation.is_finite() {
        Ok(())
    } else {
        Err(OslError::InvalidInput(format!(
            "{context} rotation must be finite"
        )))
    }
}

/// validate size。
pub(crate) fn validate_size(size: NspSize, context: &str) -> OslResult<()> {
    if size.width.is_finite() && size.height.is_finite() && size.width > 0.0 && size.height > 0.0 {
        Ok(())
    } else {
        Err(OslError::InvalidInput(format!(
            "{context} size must contain finite positive width and height"
        )))
    }
}

/// validate bus entry size。
pub(crate) fn validate_bus_entry_size(size: NspSize, context: &str) -> OslResult<()> {
    if is_valid_bus_entry_size(size) {
        Ok(())
    } else {
        Err(OslError::InvalidInput(format!(
            "{context} size must contain finite non-zero x and y deltas"
        )))
    }
}

/// is valid bus entry size。
pub(crate) fn is_valid_bus_entry_size(size: NspSize) -> bool {
    size.width.is_finite()
        && size.height.is_finite()
        && coordinate_key(size.width) != 0
        && coordinate_key(size.height) != 0
}

/// points payload。
pub(crate) fn points_payload(points: &[NspPoint]) -> String {
    points
        .iter()
        .map(|point| format!("{},{}", format_number(point.x), format_number(point.y)))
        .collect::<Vec<_>>()
        .join(";")
}

/// fnv1a64。
pub(crate) fn fnv1a64(input: &str) -> u64 {
    let mut hash = 0xcbf29ce484222325_u64;
    for byte in input.as_bytes() {
        hash ^= u64::from(*byte);
        hash = hash.wrapping_mul(0x100000001b3);
    }
    hash
}

/// uuid from hashes。
pub(crate) fn uuid_from_hashes(left: u64, right: u64) -> String {
    let mut bytes = [0_u8; 16];
    bytes[..8].copy_from_slice(&left.to_be_bytes());
    bytes[8..].copy_from_slice(&right.to_be_bytes());
    bytes[6] = (bytes[6] & 0x0f) | 0x40;
    bytes[8] = (bytes[8] & 0x3f) | 0x80;

    format!(
        "{:02x}{:02x}{:02x}{:02x}-{:02x}{:02x}-{:02x}{:02x}-{:02x}{:02x}-{:02x}{:02x}{:02x}{:02x}{:02x}{:02x}",
        bytes[0],
        bytes[1],
        bytes[2],
        bytes[3],
        bytes[4],
        bytes[5],
        bytes[6],
        bytes[7],
        bytes[8],
        bytes[9],
        bytes[10],
        bytes[11],
        bytes[12],
        bytes[13],
        bytes[14],
        bytes[15]
    )
}
