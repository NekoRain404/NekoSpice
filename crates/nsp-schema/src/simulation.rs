//! schema simulation directive parsing - .tran, .ac, .dc, .op, .noise, .disto, .sens.

use crate::edit::validate_at;
use crate::{NspAt, NspEditSummary, NspSchematic, NspTextItem};
use nsp_core::{OslError, OslResult};
use std::fmt;
use std::str::FromStr;

#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord)]
pub enum NspSimulationDirectiveKind {
    Tran,
    Ac,
    Dc,
    Op,
...
impl NspSimulationDirectiveKind {
    /// keyword。
...
impl fmt::Display for NspSimulationDirectiveKind {
...
impl FromStr for NspSimulationDirectiveKind {
    type Err = OslError;
    ...
    fn from_str(value: &str) -> OslResult<Self> {
        ...
        match normalized.as_str() {
            ...
            _ => Err(OslError::InvalidInput(format!(
                "unsupported schema simulation directive kind '{value}'"
            ))),
        }
    }
}
...
pub struct NspSimulationDirective {
    pub text: String,
    pub kind: NspSimulationDirectiveKind,
    pub at: Option<NspAt>,
    pub uuid: Option<String>,
}

impl NspSimulationDirective {
    /// from text item。
    pub fn from_text_item(item: &NspTextItem) -> Option<Self> {
        ...
        Some(Self {
            text: text.to_string(),
            kind: NspSimulationDirectiveKind::from_directive_text(text)
                .unwrap_or(NspSimulationDirectiveKind::Other),
            at: item.at,
...
#[derive(Debug, Clone, PartialEq)]
pub struct NspSimulationDirectiveUpdate {
    pub kind: NspSimulationDirectiveKind,
    pub body: String,
    pub at: Option<NspAt>,
    pub uuid: Option<String>,
...
impl NspSimulationDirectiveUpdate {
    /// normalized text。
...
pub(crate) fn simulation_directives_from_text_items(
    text_items: &[NspTextItem],
) -> Vec<NspSimulationDirective> {
    text_items
        .iter()
        .filter_map(NspSimulationDirective::from_text_item)
        .collect()
}

impl NspSchematic {
    /// simulation directives。
    pub fn simulation_directives(&self) -> Vec<NspSimulationDirective> {
        simulation_directives_from_text_items(&self.text_items)
    }

    /// spice directives。
    pub fn spice_directives(&self) -> Vec<&NspTextItem> {
        self.text_items
            .iter()
    ...
    pub fn set_simulation_directive(
        &mut self,
        update: NspSimulationDirectiveUpdate,
...
pub(crate) fn find_simulation_directive_index(
    ...
    if kind == NspSimulationDirectiveKind::Other {
        return None;
    }
    text_items
        .iter()
        .position(|item| NspSimulationDirectiveKind::from_directive_text(&item.text) == Some(kind))
}
...
pub(crate) fn normalize_simulation_directive_text(
    ...
    if body.is_empty() {
        return match kind {
            NspSimulationDirectiveKind::Op => Ok(".op".to_string()),
            _ => Err(OslError::InvalidInput(format!(
                "schema simulation directive '{kind}' body must not be empty"
            ))),
        };
    }

    if body.starts_with('.') {
        let parsed_kind = NspSimulationDirectiveKind::from_directive_text(body)
            .unwrap_or(NspSimulationDirectiveKind::Other);
        if kind != NspSimulationDirectiveKind::Other && parsed_kind != kind {
            return Err(OslError::InvalidInput(format!(
                "schema simulation directive text '{}' does not match requested kind '{}'",
                body, kind
            )));
        }
    ...
    let Some(keyword) = kind.keyword() else {
        return Err(OslError::InvalidInput(
            "schema raw simulation directives must start with '.'".to_string(),
        ));
    };
...
pub(crate) fn is_spice_analysis_directive_text(text: &str) -> bool {
    NspSimulationDirectiveKind::from_directive_text(text).is_some_and(|kind| kind.is_analysis())
}
...
fn default_simulation_directive_at(schematic: &NspSchematic) -> NspAt {
    let mut y = 20.32;
