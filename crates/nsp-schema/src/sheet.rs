//! schema hierarchical sheet parsing and pin management.

use crate::coordinates::{NspAt, NspPoint, NspSize, parse_at, parse_size};
use crate::geometry::NspBoundingBox;
use crate::instances::{
...
/// parse sheet。
pub(crate) fn parse_sheet(node: &Sexp) -> Option<NspSheet> {
...
fn parse_sheet_pin(node: &Sexp) -> Option<NspSheetPin> {
    let items = list_items(node);
    Some(NspSheetPin {
