//! schema property parsing and serialization for symbols.
...
/// parse property。
pub(crate) fn parse_property(node: &Sexp) -> Option<NspProperty> {
    ...
    Some(NspProperty {
        ...
        hide: child_value(items, "hide").and_then(parse_bool_value),
        show_name: child_value(items, "show_name").and_then(parse_bool_value),
        do_not_autoplace: child_value(items, "do_not_autoplace").and_then(parse_bool_value),
