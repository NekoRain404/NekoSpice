...
pub(crate) fn finalize_run_output(output_dir: &Path, metadata: &mut RunMetadata) -> OslResult<()> {
    finalize_run_artifacts(output_dir, metadata)
}

pub(crate) fn positional<'a>(args: &'a [String], index: usize, missing: &str) -> OslResult<&'a str> {
    positionals(args)
        .into_iter()
        .nth(index)
...
pub(crate) fn parse_jobs(value: &str) -> OslResult<usize> {
...
pub(crate) fn default_run_dir(input: &str) -> PathBuf {
