...
use nsp_core::{
    OslError, OslResult, ParameterOverride, RunMetadata, make_run_id, read_text, write_text,
};
use nsp_model::{ModelCheckOptions, ModelCheckReport};
use nsp_netlist::{ImportReport, NormalizedDependency, read_import_input};
use nsp_render::render_schema_scene_svg;
use nsp_report::{
...
use nsp_schema::{
    NspCanvasScene, NspSymbolLibraryIndexQuery, read_project, read_schematic_with_libraries,
    read_symbol_library, read_symbol_library_index, read_symbol_library_table, write_schematic,
    write_symbol_library,
};
use nsp_sim::{NgspiceCliBackend, SimulatorBackend, finalize_run_artifacts};
use nsp_waveform::{
...
use cli_run::{run_command, run_schematic_command};
use schema_edit::{parse_edit_ops, parse_point};
...
fn run_cli() -> OslResult<i32> {
    ...
    match command.as_str() {
        ...
        "run-schematic" => run_schematic_command(&args),
        "verify" => verify_command(&args),
        "bench" => bench_command(&args),
        "model-check" => model_check_command(&args),
        "import" => import_command(&args),
        "schema-inspect" => nsp_inspect_command(&args),
        "schema-select" => nsp_select_command(&args),
        "schema-check" => nsp_check_command(&args),
        "schema-export" => nsp_export_command(&args),
        "schema-edit" => nsp_edit_command(&args),
        "schema-render" => nsp_render_command(&args),
        "waveform" => waveform_command(&args),
        "report" => report_command(&args),
        unknown => Err(OslError::InvalidInput(format!(
            "unknown command '{unknown}'. Run 'nk help'."
        ))),
    }
}
...
Usage:
  nk run <netlist.cir> [--output <dir>] [--ngspice <path>]
  nk run-schematic <file.nsp_sch> [--output <dir>] [--ngspice <path>]
  nk verify <project.osl.yaml> [--output <dir>] [--ngspice <path>] [--jobs <n>]
  nk bench <directory> [--output <dir>] [--ngspice <path>]
  nk model-check <netlist-or-directory> [--output <dir>] [--symbol <ltspice.asy>]
  nk import <spice-netlist-or-ltspice.asc-or-schema_sch-or-schema-project> [--output <dir>]
  nk schema-inspect <file.nsp_pro-or-file.nsp_sch-or-file.nsp_sym-or-sym-lib-table> [--canvas] [--index] [--output <file>]
  nk schema-select <file.nsp_sch> <x,y> [--output <file>]
  nk schema-check <file.nsp_sch> [--output <file>]
  nk schema-export <file.nsp_sch-or-file.nsp_sym> --output <file>
  nk schema-edit <file.nsp_sch> --output <file.nsp_sch> [--library <file.nsp_sym>] <ops...>
      delete-item:<uuid>
  ...
  nk schema-render <file.nsp_sch-or-file.nsp_sym> [--symbol <name>] [--unit <n>] [--body-style <n>] --output <file.svg>
  nk waveform <waveform.raw> --signal <name> [--from <time>] [--to <time>] [--points <n>] [--output <file>]
  nk report <run-or-verify-dir>
  nk --version
...
#[cfg(test)]
mod tests {
    use crate::schema_edit::{parse_edit_ops, parse_point};
    ...
    use nsp_schema::{
        NspLabelKind, NspSchematicEdit, NspSimulationDirectiveKind, parse_symbol_library,
    };
    ...
    #[test]
    fn parses_schema_edit_positionals_after_output_flag() {
        let args = [
            "input.nsp_sch",
            "--output",
            "edited.nsp_sch",
        ...
        let edits = parse_edit_ops(&args, &[]).unwrap();
        assert_eq!(edits.len(), 10);
        match &edits[0] {
            NspSchematicEdit::MoveSymbol { reference, to, .. } => {
        ...
        match &edits[1] {
            NspSchematicEdit::MoveItem { uuid, delta } => {
        ...
        match &edits[3] {
            NspSchematicEdit::AddBus { points, .. } => {
        ...
        match &edits[4] {
            NspSchematicEdit::AddBusEntry { at, size, .. } => {
        ...
        match &edits[5] {
            NspSchematicEdit::AddJunction { at, .. } => {
        ...
        match &edits[6] {
            NspSchematicEdit::AddNoConnect { at, .. } => {
                assert_close(at.x, 101.6);
                assert_close(at.y, 45.72);
            }
        ...
        match &edits[7] {
            NspSchematicEdit::AddLabel { text, kind, .. } => {
                assert_eq!(text, "sense");
                assert_eq!(*kind, NspLabelKind::Global);
            }
        ...
        match &edits[8] {
            NspSchematicEdit::SetSimulationDirective {
                ...
                assert_eq!(*kind, NspSimulationDirectiveKind::Tran);
        ...
        match &edits[9] {
            NspSchematicEdit::DeleteItem { uuid } => {
                assert_eq!(uuid, "22222222-2222-2222-2222-222222222222");
            }
    ...
    #[test]
    fn parses_schema_inspect_index_flag_without_extra_positionals() {
        let args = [
    ...
    #[test]
    fn parses_schema_select_point_after_output_flag() {
        let args = [
            "input.nsp_sch".to_string(),
            "88.9,50.8".to_string(),
            "--output".to_string(),
            "hits.json".to_string(),
        ];

        assert_eq!(positionals(&args), vec!["input.nsp_sch", "88.9,50.8"]);
        assert_eq!(flag_value(&args, "--output"), Some("hits.json".to_string()));
        let point = parse_point(positionals(&args)[1], "select point").unwrap();
        assert_close(point.x, 88.9);
        assert_close(point.y, 50.8);
    }

    #[test]
    fn parses_schema_render_symbol_preview_flags_without_extra_positionals() {
        let args = [
            "library.nsp_sym".to_string(),
            "--symbol".to_string(),
            "Device:R".to_string(),
            "--unit".to_string(),
            "2".to_string(),
        ...
        assert_eq!(positionals(&args), vec!["library.nsp_sym"]);
    ...
    #[test]
    fn parses_schema_place_symbol_edit_with_library_definition() {
  ...
  (symbol "NekoSpice:C"
    ...
    (symbol "C_0_1"
      ...
      (pin passive line (at 0 2.54 270) (length 2.54) (name "~") (number "2"))
        ...
        let edits = parse_edit_ops(&args, &library.symbols).unwrap();
    ...
    #[test]
    fn parses_schema_configure_symbol_edit() {
        let args = [
            "input.nsp_sch",
            "--output",
            "configured.nsp_sch",
    ...
    #[test]
    fn parses_schema_add_sheet_edit() {
        let args = [
            "input.nsp_sch",
            "--output",
            "hierarchical.nsp_sch",
            "add-sheet:gain_stage:gain_stage.nsp_sch:66.04,45.72:25.4,10.16:in@66.04,50.8,180,input;out@91.44,50.8,0,output:uuid=30000000-0000-0000-0000-000000000008",
        ]
        .iter()
        .map(|value| value.to_string())
        .collect::<Vec<_>>();
        ...
        match &edits[0] {
            NspSchematicEdit::AddSheet {
                ...
                assert_eq!(file, "gain_stage.nsp_sch");
