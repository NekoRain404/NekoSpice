...
| `import` | Import SPICE/LTspice netlists into NekoSpice project |
| `waveform` | Query raw waveforms into min/max envelope JSON |
| `nsp-inspect` | Parse schematic files and emit JSON summary |
| `schema-check` | Run schematic diagnostics (DRC/ERC) |
| `nsp-edit` | Apply edit commands to schematics |
| `nsp-export` | Write format-compatible files from IR |
