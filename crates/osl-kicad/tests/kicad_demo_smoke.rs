use osl_kicad::{
    KicadCanvasScene, KicadDiagnosticSeverity, read_kicad_project,
    read_kicad_schematic_with_libraries, read_kicad_symbol_library_index,
};
use std::env;
...
const DEMO_CASES: &[DemoCase] = &[
    ...
    DemoCase {
        ...
        schematic: "complex_hierarchy/complex_hierarchy.kicad_sch",
    ...
    DemoCase {
        name: "cm5_minima",
        project: "cm5_minima/CM5_MINIMA_3.kicad_pro",
        schematic: "cm5_minima/CM5_MINIMA_3.kicad_sch",
...
#[test]
fn opens_representative_kicad_demo_projects_from_reference_source() {
    let Some(demo_root) = kicad_demo_root() else {
        eprintln!(
            "skipping KiCad demo smoke test: set NEKOSPICE_KICAD_DEMOS to a KiCad demos directory"
        );
        return;
    };
...
fn smoke_demo_case(demo_root: &Path, case: &DemoCase) {
    ...
    let project = read_kicad_project(&project_path)
    ...
    let library_index = read_kicad_symbol_library_index(&library_table_path)
    ...
    assert!(
        library_index
            .diagnostics
            .iter()
            .all(|diagnostic| diagnostic.severity != KicadDiagnosticSeverity::Error),
        "{} symbol library index returned error diagnostics: {:?}",
        case.name,
        library_index.diagnostics
    );

    let schematic = read_kicad_schematic_with_libraries(&schematic_path)
...
fn assert_scene_baseline(case: &DemoCase, scene: &KicadCanvasScene) {
    let pin_count = scene
        .symbols
        .iter()
    ...
    if case.expect_bus {
        assert!(
            ...
            "{} should expose KiCad bus geometry",
            case.name
        );
    }
