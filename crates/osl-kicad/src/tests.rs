use super::{
    KicadAt, KicadBoundingBox, KicadCanvasScene, KicadColor, KicadDiagnosticSeverity, KicadGraphic,
    KicadIndexedSymbolBodyStyle, KicadIndexedSymbolUnit, KicadLabelKind, KicadPoint,
    KicadSchematicEdit, KicadSheetPin, KicadSimulationDirectiveKind, KicadSize,
    KicadSymbolBodyStyles, KicadSymbolLibraryIndexQuery, KicadSymbolPlacement, KicadSymbolPower,
...
fn preserves_symbol_and_sheet_assembly_flags() {
  ...
  (sheet
    ...
    assert!(roundtrip.contains("(on_board no)"));
    let reparsed = parse_kicad_schematic(&roundtrip, "assembly_flags_roundtrip.kicad_sch").unwrap();
...
fn preserves_label_shape_autoplace_and_properties() {
  ...
  (hierarchical_label "CHILD_IN"
    ...
    let reparsed = parse_kicad_schematic(&roundtrip, "label_metadata_roundtrip.kicad_sch").unwrap();
...
fn parses_hierarchical_sheet_items_and_reports_unsupported_expansion() {
  ...
  (sheet
    ...
    let netlist = schematic.to_spice_netlist().unwrap();
    assert!(
        netlist.contains("* Unsupported KiCad hierarchical sheet gain_stage gain_stage.kicad_sch")
    );
    let roundtrip = schematic.to_kicad_schematic_sexpr();
    ...
    let reparsed = parse_kicad_schematic(&roundtrip, "hierarchical_roundtrip.kicad_sch").unwrap();
    assert_eq!(
        reparsed.sheets[0]
            .stroke
            .as_ref()
...
fn edits_kicad_sheet_pins_by_uuid_and_roundtrips() {
  ...
  (sheet
    ...
    let reparsed = parse_kicad_schematic(&exported, "sheet_pin_edits_roundtrip.kicad_sch").unwrap();
    assert_eq!(reparsed.sheets.len(), 1);
...
fn preserves_kicad_symbol_library_file_metadata() {
  ...
  (symbol "NekoSpice:Embedded"
    ...
    let reparsed = parse_kicad_symbol_library(&exported, "metadata_roundtrip.kicad_sym").unwrap();
...
fn preserves_kicad_symbol_pin_display_and_text_effects() {
  ...
  (symbol
    ...
    let reparsed = parse_kicad_symbol_library(&exported, "styled_pin_roundtrip.kicad_sym").unwrap();
...
fn preserves_kicad_symbol_pin_alternates_and_canvas_metadata() {
  ...
  (symbol
    ...
    let reparsed = parse_kicad_symbol_library(&exported, "alt_pin_roundtrip.kicad_sym").unwrap();
    assert_eq!(
...
fn preserves_kicad_symbol_inheritance_body_styles_and_jumpers() {
  ...
  (symbol "NekoSpice:Derived"
    ...
    let reparsed =
        parse_kicad_symbol_library(&exported, "symbol_inheritance_roundtrip.kicad_sym").unwrap();
    assert_eq!(
        reparsed
            .symbol("NekoSpice:Derived")
            .unwrap()
...
fn parses_kicad_project_fixture_and_sheet_summary() {
    ...
    let project = read_kicad_project(
        &workspace_root.join("examples/kicad_project_schematic/kicad_project_schematic.kicad_pro"),
    )
    .unwrap();
...
fn indexes_kicad_symbol_library_browser_metadata() {
  ...
  (lib (name "Browser")(type "KiCad")(uri "${KIPRJMOD}/browser.kicad_sym")(options "")(descr ""))
    ...
    let reparsed = parse_kicad_symbol_library(&exported, "browser_roundtrip.kicad_sym").unwrap();
    assert_eq!(
