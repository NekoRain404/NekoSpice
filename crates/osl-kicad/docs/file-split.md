# osl-kicad File Split

## Overview

The `osl-kicad` crate provides KiCad `.kicad_sch`/`.kicad_sym`/`.kicad_pro` parsing, canvas scene generation, schematic editing, and geometry utilities.
...
| `src/canvas_items_graphic_impl.rs` | ~343 | `KicadCanvasGraphic` enum (Polyline, Bezier, Rectangle, Circle, Arc, Text) and impl |
...
- **Arc rendering**: Uses `sample_kicad_arc_points()` for proper arc interpolation instead of 3-point V-shape
- **Bezier rendering**: Quadratic bezier approximation with 24-segment sampling
- **Text rendering**: Respects KiCad `font_size` from text effects, uses proportional font
