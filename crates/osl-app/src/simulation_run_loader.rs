...
mod tests {
    ...
    use crate::waveform_summary::GuiWaveformSummaryState;
    use osl_core::{Artifact, RunMetadata, RunStatus};
    use std::fs;
...
"#;
    ...
    fn loads_existing_run_directory_for_gui_smoke() {
        ...
        osl_core::write_text(&output_dir.join("waveform.raw"), SAMPLE_RAW).unwrap();
        osl_sim::finalize_run_artifacts(&output_dir, &mut metadata).unwrap();
        metadata.duration_ms = 7;
        osl_core::write_text(&output_dir.join("run.json"), &metadata.to_json()).unwrap();
