...
use super::simulation_profile_editor_widgets::{param_table, section_header};
use super::simulation_workspace_widgets::analysis_mode_button;
use super::theme::StudioTheme;
use eframe::egui;
use osl_kicad::KicadSimulationDirectiveKind;

/// Draw the analysis setup panel (left column, top).
pub(super) fn draw_analysis_setup_panel(app: &mut NekoSpiceApp, ui: &mut egui::Ui) {
    ...
    StudioTheme::panel_frame_for(mode).show(ui, |ui| {
        ...
        ui.label(StudioTheme::muted_for(mode, app.text(UiText::Body)));
        ui.horizontal(|ui| {
            ui.text_edit_singleline(&mut app.simulation_panel.directive_body);
            if ui
                .add_enabled(
                    app.document.is_some(),
                    egui::Button::new(app.text(UiText::SetDirective)),
                )
                .clicked()
            {
                app.apply_simulation_directive_edit();
            }
        });
    });
}
...
pub(super) fn draw_component_params(app: &mut NekoSpiceApp, ui: &mut egui::Ui) {
    let mode = app.theme_mode();
    StudioTheme::panel_frame_for(mode).show(ui, |ui| {
...
pub(super) fn draw_model_params(app: &mut NekoSpiceApp, ui: &mut egui::Ui) {
    let mode = app.theme_mode();
    StudioTheme::panel_frame_for(mode).show(ui, |ui| {
...
pub(super) fn draw_parameter_definitions(app: &mut NekoSpiceApp, ui: &mut egui::Ui) {
    let mode = app.theme_mode();
    StudioTheme::panel_frame_for(mode).show(ui, |ui| {
        ...
        // Editable parameter definition rows (uses the same component_params from state)
