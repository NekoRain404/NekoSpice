...
pub(super) fn progress_bar(ui: &mut egui::Ui, mode: StudioThemeMode, label: &str, value: f32) {
    let palette = StudioTheme::palette(mode);
    ui.horizontal(|ui| {
        ui.label(label);
...
pub(super) fn mini_donut(ui: &mut egui::Ui, mode: StudioThemeMode, pass_ratio: f32) {
    let palette = StudioTheme::palette(mode);
