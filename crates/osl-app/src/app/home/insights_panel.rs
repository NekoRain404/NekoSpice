        ...
        StudioTheme::panel_frame_for(mode).show(ui, |ui| {
            ...
            if error_count > 0 {
                if insight_row(
                    ...
                    mode,
                    stability_title,
                    &format!("{error_count} ERC errors found — review schematic"),
                    view_label,
                ) {
                ...
                && insight_row(
                    ...
                    mode,
                    stability_title,
                    &format!("{warning_count} ERC warnings — consider fixing"),
                    view_label,
                )
            {
                self.active_workspace = crate::app::navigation::StudioWorkspace::Review;
            ...
            if has_run && waveform_vars > 0 {
                if insight_row(
                    ...
                    mode,
                    thd_title,
                    &format!("Simulation complete — {waveform_vars} signals captured"),
                    view_label,
                ) {
                ...
                && insight_row(
                    ...
                    mode,
                    thd_title,
                    "Last simulation failed — check log for errors",
                    view_label,
                )
            {
                self.active_workspace = crate::app::navigation::StudioWorkspace::Simulation;
            ...
            if has_doc && !has_run {
                if insight_row(
                    ...
                    mode,
                    noise_title,
                    "Ready to simulate — run a simulation to validate",
                    view_label,
                ) {
                    self.active_workspace = crate::app::navigation::StudioWorkspace::Simulation;
                }
            ...
            if insight_row(
                ...
                mode,
                power_title,
                &format!("System: {threads} threads, wgpu renderer active"),
                view_label,
            ) {
