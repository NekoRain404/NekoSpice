...
use super::widgets::{
    measurement_row, project_row, queue_row, recommendation_row, section_header,
    section_header_clickable,
};
                    ...
                    &run.waveform
                {
                    for variable in summary.variables.iter().take(4) {
                        let value_str = if !variable.unit.is_empty() {
                            format!(
                                "{} {}",
                                super::super::waveform::preview_primitives::format_compact_f64(
                                    variable.last
                                ),
                                variable.unit
                            )
        ...
        StudioTheme::panel_frame_for(mode).show(ui, |ui| {
            ...
            if self.document.is_none() {
                if recommendation_row(
                    ui,
                    mode,
                    "Open a schematic to get started",
                    self.text(UiText::Open),
                ) {
                    self.active_workspace = StudioWorkspace::Schematic;
                }
            } else {
                ...
                if !has_run && !running {
                    if recommendation_row(
                        ui,
                        mode,
                        "Run a simulation to validate your design",
                    ...
                    if recommendation_row(
                        ui,
                        mode,
                        "Fix simulation errors — check log output",
                        self.text(UiText::ViewAll),
                    ) {
                        self.active_workspace = StudioWorkspace::Simulation;
                    }
                    ...
                    if recommendation_row(
                        ...
                        &format!(
                            ...
                            self.simulation_panel.netlist_warnings.len()
                        ),
                        self.text(UiText::ViewAll),
                    ) {
                        self.active_workspace = StudioWorkspace::Simulation;
                ...
                if let Some(r) = report
                    ...
                    && recommendation_row(
                        ui,
                        mode,
                        &format!("Fix {} ERC errors in schematic", r.error_count()),
                        self.text(UiText::ViewAll),
                    )
                {
                    self.active_workspace = StudioWorkspace::Schematic;
                ...
                if recommendation_row(
                    ...
                    self.simulation_panel.step_sweep =
                        super::super::simulation::state::StepSweep::Parametric {
