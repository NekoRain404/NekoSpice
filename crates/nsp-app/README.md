...
renderer selected explicitly, while schematic parsing, editing, and symbol-library
search stay in `nsp-schema`.
...
- `app/library_workspace.rs`: Library center workspace layout, header, search,
  and category tabs for format-compatible symbols.
...
- `app/library_preview.rs`: Library-only preview drawing for symbol canvas
...
- `app/selection_properties.rs`: selected symbol property editor state sync and `NspSchematicEdit::{SetSymbolProperty, ConfigureSymbol}` routing.
...
- `app/schematic_inspector_panel.rs`: Schematic right workspace tab state and
  routing for properties, Schema inspection, libraries, and simulator context.
- `app/schematic_inspector_sections.rs`: Schematic right workspace properties,
  cross-probe actions, Schema document structure, diagnostics, and library scope
  sections.
...
- `app/symbol_placement_controls.rs`: unit, body-style, and pin-alternate controls for Schema-compatible symbol placement.
...
- `document.rs`: editable schematic adapter around `NspSchematicEdit`,
...
- `waveform_summary.rs`: GUI-facing waveform summary adapter around
  ...
  panel drawing code.
- `library.rs`: GUI-facing symbol-library table, definition, dependency, and preview adapter around `NspSymbolLibraryIndex`.
...
- `canvas.rs`: Schema canvas scene traversal and draw-order routing.
- `canvas/primitives.rs`: current egui painter prototype for grid, shapes, graphics, and bounds.

## Rules

- UI code may call document/library adapters, but should not parse schematic files directly.
...
- Schema geometry, hit testing, and edit semantics belong in `nsp-schema`; GUI code consumes those APIs.
- Canvas input handling belongs in `app/canvas_panel.rs`; drawing primitives belong in `canvas/primitives.rs`.
- Selection property editing reads selected canvas metadata from `NspCanvasScene` and writes only through `document.rs`.
- Symbol placement starts in the library/document adapters, then the UI wires selection, scope controls, preview, and canvas clicks to those adapters; repeat placement is UI state only, not Schema file logic.
- Schematic drawing tools keep transient UI state under `app/schematic_tools/` and route all file mutations through `document.rs`, which in turn calls `NspSchematicEdit`.
- Simulation setup UI should stay in GUI state and route Schema-compatible directive changes through `document.rs`, not by editing text items directly.
