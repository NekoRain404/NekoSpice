...
renderer selected explicitly, while Schema parsing, editing, and symbol-library
search stay in `osl-schema`.
...
- `app/library_workspace.rs`: Library center workspace layout, header, search,
  and category tabs for Schema-compatible symbols.
...
- `app/library_preview.rs`: Library-only preview drawing for Schema symbol canvas
...
- `document.rs`: editable Schema schematic adapter around `KicadSchematicEdit`,
...
- UI code may call document/library adapters, but should not parse Schema files directly.
...
- Schema geometry, hit testing, and edit semantics belong in `osl-schema`; GUI code consumes those APIs.
