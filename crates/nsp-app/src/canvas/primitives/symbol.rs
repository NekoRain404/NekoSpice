...
/// Resolve font size from schema text effects, with a minimum of 6pt.
fn resolve_font_size(effects: &Option<nsp_schema::NspTextEffects>) -> f32 {
    effects
        .as_ref()
...
use nsp_schema::{NspCanvasGraphic, sample_arc_points};

/// Returns at least `min_width` pixels for visibility.
fn resolve_stroke_width(
    stroke: &Option<nsp_schema::NspStroke>,
...
/// Draw a schema canvas graphic element (polyline, bezier, rectangle, circle, arc, text).
///
/// Supports solid fills via schema fill data. Filled rectangles and circles
/// get a translucent background fill matching the schema rendering style.
/// Stroke widths are resolved from schema data when available.
