...
It converts canvas scene data into visual output using egui's painter API.

## Architecture

```text
NspSchematic → NspCanvasScene → draw_scene(colors) → egui Painter
```
...
- **Light theme**: standard colors on light-gray background
...
5. **Wires** — green polylines with standard stroke width
...
- **Stroke width resolution**: Wire/bus/graphic stroke widths are read from format data.
  When format specifies `(stroke (width 0))`, the renderer defaults to 0.1524mm (6mil).
...
- World coordinates: standard mm units (origin top-left, Y increases downward)
