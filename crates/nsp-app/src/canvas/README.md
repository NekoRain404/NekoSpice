...
It converts Schema canvas scene data into visual output using egui's painter API.
...
- **Light theme**: Schema-standard colors on light-gray background
...
5. **Wires** — green polylines with Schema-default stroke width
...
- **Stroke width resolution**: Wire/bus/graphic stroke widths are read from Schema data.
  When Schema specifies `(stroke (width 0))`, the renderer defaults to 0.1524mm (6mil).
...
- World coordinates: Schema mm units (origin top-left, Y increases downward)
