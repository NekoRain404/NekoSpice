...
use nsp_schema::{NspBoundingBox, NspPoint};
...
impl CanvasViewport {
    /// for rect。
    pub(crate) fn for_rect(rect: Rect, bounds: Option<NspBoundingBox>) -> Self {
    ...
    /// fit scene。
    pub(crate) fn fit_scene(&mut self, bounds: Option<NspBoundingBox>) {
        self.fit_size(Vec2::new(900.0, 560.0), bounds, 4.0..=32.0);
    }

    /// fit rect。
    pub(crate) fn fit_rect(&mut self, rect: Rect, bounds: Option<NspBoundingBox>) {
    ...
    /// world to screen。
    pub(crate) fn world_to_screen(self, rect: Rect, point: NspPoint) -> Pos2 {
        rect.center() + self.pan + Vec2::new(point.x as f32 * self.zoom, point.y as f32 * self.zoom)
    }

    /// screen to world。
    pub(crate) fn screen_to_world(self, rect: Rect, point: Pos2) -> NspPoint {
    ...
    pub(crate) fn visible_world_bounds(self, rect: Rect) -> NspBoundingBox {
...
/// item visible。
pub(crate) fn item_visible(bounds: Option<NspBoundingBox>, visible_bounds: NspBoundingBox) -> bool {
    // Scene geometry stays in osl-schema; the GUI only decides whether an item can affect this viewport.
...
mod tests {
    ...
    fn viewport_roundtrips_screen_and_world_points() {
        ...
        let world = NspPoint { x: 12.7, y: 5.08 };
    ...
    fn viewport_exposes_visible_world_bounds_for_canvas_culling() {
        ...
        assert!(item_visible(
            Some(NspBoundingBox {
                min: NspPoint { x: 9.0, y: 4.0 },
                max: NspPoint { x: 12.0, y: 6.0 },
            }),
            visible
        ));
        assert!(!item_visible(
            Some(NspBoundingBox {
                min: NspPoint { x: 12.0, y: 6.0 },
                max: NspPoint { x: 14.0, y: 8.0 },
            }),
            visible
        ));
    }
