...
use eframe::egui;
use nsp_schema::NspCanvasSymbol;
...
impl SelectedSymbolPropertySnapshot {
    fn from_symbol(symbol: &NspCanvasSymbol) -> Self {
        Self {
            reference: symbol.reference.clone(),
            value: symbol.value.clone(),
            mirror: SymbolMirrorSelection::from_schema(symbol.mirror.as_deref()),
        }
    }
}
...
impl SymbolMirrorSelection {
    ...
    fn from_schema(mirror: Option<&str>) -> Self {
        match mirror {
            Some("x") => Self::X,
            Some("y") => Self::Y,
...
impl NekoSpiceApp {
    ...
    fn apply_selected_symbol_properties(&mut self, original: SelectedSymbolPropertySnapshot) {
        ...
        let mirror = self.selection_properties.mirror.to_schema();
    ...
    fn selected_symbol_for_property_editor(&self) -> Option<&NspCanvasSymbol> {
