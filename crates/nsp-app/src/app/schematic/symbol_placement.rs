...
use eframe::egui;
use nsp_schema::{NspIndexedSymbolBodyStyle, NspIndexedSymbolPin, NspIndexedSymbolUnit};
...
impl NekoSpiceApp {
    ...
    fn draw_pin_alternate_controls(&mut self, ui: &mut egui::Ui, pins: &[NspIndexedSymbolPin]) {
    ...
    fn retain_valid_pin_alternates(&mut self, pins: &[NspIndexedSymbolPin]) {
...
fn draw_unit_selector(ui: &mut egui::Ui, selected_unit: &mut u32, units: &[NspIndexedSymbolUnit]) {
    if units.is_empty() {
        return;
    }
...
fn draw_body_style_selector(
    ...
    body_styles: &[NspIndexedSymbolBodyStyle],
) {
...
fn format_unit_label(unit: u32, units: &[NspIndexedSymbolUnit]) -> String {
    units
        .iter()
...
fn format_body_style_label(body_style: u32, body_styles: &[NspIndexedSymbolBodyStyle]) -> String {
    body_styles
        .iter()
...
fn scoped_alternate_pins<'a>(
    pins: &'a [NspIndexedSymbolPin],
    config: &crate::placement_config::SymbolPlacementConfig,
) -> Vec<&'a NspIndexedSymbolPin> {
    pins.iter()
        .filter(|pin| {
...
fn format_pin_alternate_label(pin: &NspIndexedSymbolPin, selected: &str) -> String {
    if selected.is_empty() {
...
mod tests {
    ...
    use nsp_schema::NspPinAlternate;
