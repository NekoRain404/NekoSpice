...
use nsp_schema::{NspAt, NspEditSummary, NspLabelKind, NspPoint, NspSheetPin, NspSize};
...
impl NekoSpiceApp {
    ...
    pub(in crate::app) fn handle_schematic_tool_click(&mut self, point: NspPoint) -> bool {
        match self.schematic_tools.active {
            ...
            SchematicTool::Label => {
                ...
                self.apply_schematic_tool_edit(Some(point), |document| {
                    document.add_label(text, NspLabelKind::Local, at_from_point(point))
                });
                true
            }
            SchematicTool::GlobalLabel => {
                let text = self.schematic_tools.label_text.clone();
                self.apply_schematic_tool_edit(Some(point), |document| {
                    document.add_label(text, NspLabelKind::Global, at_from_point(point))
                });
                true
            }
            SchematicTool::HierarchicalLabel => {
                let text = self.schematic_tools.label_text.clone();
                self.apply_schematic_tool_edit(Some(point), |document| {
                    document.add_label(text, NspLabelKind::Hierarchical, at_from_point(point))
                });
                true
            }
            SchematicTool::Text => {
    ...
    fn handle_wire_tool_click(&mut self, point: NspPoint) {
    ...
    fn handle_bus_tool_click(&mut self, point: NspPoint) {
    ...
    fn apply_schematic_tool_edit<F>(&mut self, selection_point: Option<NspPoint>, edit: F) -> bool
    where
        F: FnOnce(&mut NspGuiDocument) -> Result<NspEditSummary, String>,
    {
...
fn at_from_point(point: NspPoint) -> NspAt {
    NspAt {
        x: point.x,
        y: point.y,
        rotation: 0.0,
...
fn sheet_pins_for_tool(point: NspPoint, size: NspSize, names: &[String; 2]) -> Vec<NspSheetPin> {
    let mut pins = Vec::new();
    let y = point.y + size.height / 2.0;
    let left = names[0].trim();
    if !left.is_empty() {
        pins.push(NspSheetPin {
            name: left.to_string(),
...
fn same_point(left: NspPoint, right: NspPoint) -> bool {
