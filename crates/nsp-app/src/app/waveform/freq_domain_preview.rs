...
//! plots using the pure-Rust FFT engine from `nsp_waveform::fft`.
//!
...
use nsp_waveform::fft::{self, WindowFunction};
...
pub(crate) fn draw_fft_magnitude_plot(
    ...
    let signal_name =
        selected_signal.or_else(|| summary.previews.first().map(|p| p.signal.as_str()));
    ...
    draw_freq_grid(
        ...
        plot_rect,
        &bins,
        palette.border.linear_multiply(0.55),
    );
...
pub(crate) fn draw_bode_plot(
    ...
    draw_freq_grid(
        ...
        mag_plot,
        &bins,
        palette.border.linear_multiply(0.5),
    );
...
pub(crate) fn draw_noise_plot(
    ...
    let noise_density: Vec<f64> = bins
        .iter()
        .map(|b| {
    ...
    let max_nd = noise_density
        .iter()
        .cloned()
        .fold(f64::NEG_INFINITY, f64::max);
    let min_nd = noise_density
        .iter()
        .cloned()
        .filter(|v| *v > -300.0)
