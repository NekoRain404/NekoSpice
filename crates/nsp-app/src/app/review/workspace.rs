    ...
    fn draw_review_score_overview(&self, ui: &mut egui::Ui) {
        ...
        StudioTheme::panel_frame_for(mode).show(ui, |ui| {
            ...
            ui.horizontal(|ui| {
                ...
                ui.vertical(|ui| {
                    review_metric_row(ui, mode, self.text(UiText::OverallScore), score_label);
                    review_metric_row(ui, mode, self.text(UiText::Critical), &err.to_string());
                    review_metric_row(ui, mode, self.text(UiText::Major), &warn.to_string());
                    review_metric_row(ui, mode, self.text(UiText::Minor), &info.to_string());
                });
            });
        });
    }
