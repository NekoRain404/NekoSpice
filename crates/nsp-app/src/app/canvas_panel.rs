...
use nsp_schema::{NspCanvasScene, read_schematic_with_libraries};
use std::path::Path;
...
impl NekoSpiceApp {
    ...
    fn draw_symbol_placement_preview(
        ...
        canvas::draw_scene(
            ...
            &preview.scene,
            self.viewport,
            preview.scene.bounds.unwrap_or(nsp_schema::NspBoundingBox {
                min: point,
                max: point,
            }),
            schematic_colors,
...
/// Load a schema schematic file and build the canvas scene for rendering.
pub fn load_canvas_scene(path: &Path) -> Result<NspCanvasScene, String> {
