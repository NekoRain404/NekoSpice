...
use nsp_sim::{ProfileParamEntry, SimulationProfile, SpiceMethod};
use std::path::Path;
...
impl NekoSpiceApp {
    ...
    pub(crate) fn build_simulation_profile(&self) -> SimulationProfile {
        ...
        let analysis_kind = match self.simulation_panel.directive_kind {
            NspSimulationDirectiveKind::Tran => ".tran".to_string(),
            NspSimulationDirectiveKind::Ac => ".ac".to_string(),
            NspSimulationDirectiveKind::Dc => ".dc".to_string(),
            NspSimulationDirectiveKind::Op => ".op".to_string(),
            NspSimulationDirectiveKind::Noise => ".noise".to_string(),
            NspSimulationDirectiveKind::Disto => ".disto".to_string(),
            NspSimulationDirectiveKind::Sens => ".sens".to_string(),
            _ => ".tran".to_string(),
        };
    ...
    pub(in crate::app) fn poll_simulation_task(&mut self) {
        ...
        match result {
            Ok(run) => {
                if run.metadata.status == nsp_core::RunStatus::Failed {
