...
impl NekoSpiceApp {
    /// Open a native file dialog to pick a schema schematic file (.kicad_sch).
    ///
    ...
    pub(super) fn open_file_dialog(&mut self) {
        let dialog = rfd::FileDialog::new()
            .add_filter("schema Schematic", &["kicad_sch"])
            .add_filter("schema Symbol Library", &["kicad_sym"])
    ...
    pub(super) fn open_library_dialog(&mut self) {
        let dialog = rfd::FileDialog::new()
            .add_filter("schema Symbol Library", &["kicad_sym", "sym-lib-table"])
            .add_filter("All Files", &["*"]);
