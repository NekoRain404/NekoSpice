...
impl NekoSpiceApp {
    ...
    pub(super) fn load_symbol_library(&mut self, path: PathBuf) {
        match NspGuiLibrary::load(path.clone()) {
            Ok(library) => {
    ...
    pub(super) fn delete_selected(&mut self) {
        let Some(uuid) = self.selected_hit.as_ref().and_then(|hit| hit.uuid.clone()) else {
            self.status_message = Some("Selected item has no schema UUID".to_string());
            return;
        };
    ...
    pub(super) fn move_selected(&mut self, delta: NspPoint) {
        let Some(uuid) = self.selected_hit.as_ref().and_then(|hit| hit.uuid.clone()) else {
            self.status_message = Some("Selected item has no schema UUID".to_string());
            return;
        };
    ...
    pub(super) fn create_new_schematic(&mut self, name: &str) {
        let document = NspGuiDocument::new_empty(name);
        let scene = document.scene();
