...
use eframe::egui;
use nsp_schema::NspIndexedSymbol;
...
impl NekoSpiceApp {
    ...
    fn draw_model_browser_header(&mut self, ui: &mut egui::Ui, symbol: &NspIndexedSymbol) {
        let mode = self.theme_mode();
        StudioTheme::panel_frame_for(mode).show(ui, |ui| {
            ui.horizontal_top(|ui| {
                ui.vertical(|ui| {
    ...
    fn draw_library_symbol_and_pins(&mut self, ui: &mut egui::Ui, symbol: &NspIndexedSymbol) {
        let mode = self.theme_mode();
        StudioTheme::panel_frame_for(mode).show(ui, |ui| {
            ui.label(StudioTheme::section_title_for(
                mode,
    ...
    fn draw_library_spice_model_card(&self, ui: &mut egui::Ui, symbol: &NspIndexedSymbol) {
        let mode = self.theme_mode();
        StudioTheme::panel_frame_for(mode).show(ui, |ui| {
            ui.label(StudioTheme::section_title_for(
                mode,
