...
use nsp_sim::{SimulationProfile, SimulatorBackend};
use std::path::Path;
...
mod unit_tests {
    use super::*;
    use nsp_core::{Artifact, ParameterOverride, RunStatus};
    use std::fs;
...
"#;
    ...
    impl SimulatorBackend for RecordingBackend {
        ...
        fn capabilities(&self) -> nsp_sim::BackendCapabilities {
            nsp_sim::BackendCapabilities {
                batch: true,
        ...
        fn run_with_parameters(
            ...
            nsp_core::write_text(&output_dir.join("waveform.raw"), SAMPLE_RAW)?;
            Ok(RunMetadata {
                schema_version: 1,
    ...
    fn writes_document_netlist_and_runs_backend() {
        ...
        let document = NspGuiDocument::load(temp.path().to_path_buf()).unwrap();
        let runs_root =
            std::env::temp_dir().join(format!("nekospice_gui_run_{}", nsp_core::now_unix_ms()));
    ...
    fn background_task_returns_backend_result() {
        ...
        let document = NspGuiDocument::load(temp.path().to_path_buf()).unwrap();
        let runs_root =
            std::env::temp_dir().join(format!("nekospice_gui_task_{}", nsp_core::now_unix_ms()));
        let job =
