...
pub fn parse_ngspice_binary_raw(input: &[u8], source_name: &str) -> OslResult<Waveform> {
    ...
    let is_complex = header.flags.iter().any(|flag| flag == "complex");
    ...
    let f64_size = if is_complex { 16 } else { 8 }; // complex: real + imag pairs
    if !payload.len().is_multiple_of(f64_size) {
        return Err(OslError::InvalidInput(format!(
            "{} binary payload length {} at byte offset {} is not aligned to {}",
            source_name,
            payload.len(),
            payload_offset,
            if is_complex { "complex f64 pairs" } else { "f64 values" }
        )));
    }

    let value_count = payload.len() / f64_size;
    if !value_count.is_multiple_of(variable_count) {
        return Err(OslError::InvalidInput(format!(
            "{} binary payload contains {} values, which is not divisible by {} variables",
