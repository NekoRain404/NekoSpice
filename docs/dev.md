...
NekoSpice-RS 是一款基于 Rust 的开源 SPICE 仿真验证平台，目标不是简单复制 LTspice / 标准格式，而是参考公开源码和文件格式，在 Rust 中重构兼容标准格式的原理图绘制与 symbol library 能力，并在以下方向形成优势：

* 高性能波形查看器
* 自动化仿真验证
* 参数扫描、Corner、Monte Carlo
* 模型兼容性诊断
* Rust-native format-compatible 原理图绘制与 symbol library
...
* Xyce CLI，后期可选
* 公开参考源码、文件格式文档和 `NekoSpice CLI` 作为参考/对照验证
* Python 插件，后期可选
...
├── nsp-schema
│   ├── S-expression parser
│   ├── .nsp_sch reader/writer
│   ├── .nsp_sym reader/writer
│   ├── schematic IR
...
│   ├── SPICE parser
│   ├── LTspice parser
│   ├── 标准格式 schematic-to-SPICE adapter
│   ├── 标准格式 netlist importer
│   └── netlist normalizer
│
├── nsp-model
│   ├── model index
...
### 阶段 6：format-compatible 原理图/库子系统与迁移导入，6–8 个月

目标：

```text
完成 .nsp_pro / .nsp_sch 初版 Rust-native 解析
完成 .nsp_sym symbol library 解析和索引
