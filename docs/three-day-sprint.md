...
- 实现 `osl import <spice-netlist-or-ltspice.asc-or-kicad-project>` 的导入报告和 normalized project 输出：组件数量、symbol 数量、directive 数量、include、兼容性评分、`project/input.cir`、`project/project.osl.yaml`、`project/manifest.json`，复制相对 `.include` / `.lib` 模型依赖，并生成可观测信号与 checks 模板建议。
...
- 准备 KiCad-style SPICE netlist fixture 和 KiCad project fixture，并确认它们可以被 ngspice 运行。
...
cargo run -p osl-cli -- import examples/kicad_project --output reports/kicad_project_dir_001
...
cargo run -p osl-cli -- verify reports/ltspice_subckt_import_001/project/project.osl.yaml --output reports/ltspice_subckt_import_001_verify
cargo run -p osl-cli -- import examples/ltspice_import/ltspice_sym_search.asc --output reports/ltspice_sym_search_001
cargo run -p osl-cli -- verify reports/ltspice_sym_search_001/project/project.osl.yaml --output reports/ltspice_sym_search_001_verify
cargo run -p osl-cli -- import examples/ltspice_import/ltspice_bjt.asc --output reports/ltspice_bjt_import_001
cargo run -p osl-cli -- verify reports/ltspice_bjt_import_001/project/project.osl.yaml --output reports/ltspice_bjt_import_001_verify
cargo run -p osl-cli -- import examples/ltspice_import/ltspice_vcvs.asc --output reports/ltspice_vcvs_import_001
cargo run -p osl-cli -- verify reports/ltspice_vcvs_import_001/project/project.osl.yaml --output reports/ltspice_vcvs_import_001_verify
...
1. Rust-native KiCad schematic/library：参考 KiCad 源码和官方 S-expression 格式，继续扩展 `.kicad_sch` / `.kicad_sym` parser、Rust IR、symbol library index、连接图、hierarchical sheet 编辑、schematic-to-SPICE 和后续 schematic canvas 数据模型，并把 symbol fields、模型路径、pin order 和仿真 directives 转为可诊断报告。
