...
- 代码管理补充：`osl-kicad` 继续以文件管理和模块边界为第一优先级，已把 schematic image / embedded base64 data、wire/bus/bus-entry/bus-alias/net-chain、junction/no-connect marker、table/table-cell、property、label/directive-label、text/text-box、group、hierarchical sheet/sheet-pin IR、shared drawing primitive / schematic graphic / rule-area IR 与 parse/write/helper 从 `lib.rs` 拆入 `image.rs`、`wiring.rs`、`markers.rs`、`table.rs`、`property.rs`、`labels.rs`、`text.rs`、`group.rs`、`sheet.rs`、`graphics.rs`。后续拆分继续按“低耦合 IR + parse/write + bounds/helper 同模块”的规则推进，根模块只保留公共 API、顶层装配和暂未拆出的复杂 symbol/netlist 逻辑。
