...
- 保留 schematic-level drawing primitives（`polyline` / `bezier` / `rectangle` / `circle` / `arc`）的 `stroke` / `fill` / `locked` 元数据，并在 symbol graphics 中保留 Bezier 曲线，确保 Rust-native 原理图画布与 KiCad 文件 roundtrip 不丢失绘制样式。
