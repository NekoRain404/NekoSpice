...
- 实现 wire、junction、label、global label、no-connect、hierarchical sheet 的连接图和基础编辑命令；保留 wire/bus/bus entry stroke 样式；实现按 UUID 删除顶层 schematic/canvas item 的 Rust-native 编辑入口；实现 bus alias、bus、bus entry 的 KiCad 资产 roundtrip、canvas 数据和基础编辑命令；保存 `net_chain` 元数据并支持 `.kicad_sch` roundtrip；bus 暂作为画布/互操作元素，不错误合并进模拟 SPICE net。
