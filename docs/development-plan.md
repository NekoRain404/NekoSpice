...
- `osl-kicad`：KiCad S-expression parser、`.kicad_sch` / `.kicad_sym` / `.kicad_pro` IR、symbol library index / library browser metadata、schematic canvas scene、hit-test/selection backend 和后续 GPU renderer 需要的稳定场景数据。
