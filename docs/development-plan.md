...
当前仓库已建立 Rust workspace、`osl` CLI、ngspice runner、验证 DSL、批量 sweep、model-check、import、waveform viewport 查询、示例电路和 Git 管理。近期路线调整为新增 KiCad schematic/library 的 Rust-native 基础层，把 KiCad 工程资产接入 NekoSpice 自有原理图、仿真、验证和波形工作流。单次仿真的 artifact finalizer 已收敛到 `osl-sim`，统一负责 `waveform.csv`、`waveform-summary.json`、`report.html` 和刷新后的 `run.json`，CLI 与 GUI 共用同一输出闭环。
...
- `osl-kicad`：KiCad S-expression parser、`.kicad_sch` / `.kicad_sym` / `.kicad_pro` IR、project JSON parser、symbol library index / library browser metadata、schematic edit command DTO/helper、schematic canvas scene、hit-test/selection backend 和后续 GPU renderer 需要的稳定场景数据。
...
- `osl-report`：共享报告 DTO、HTML/JSON/Markdown/JUnit XML renderer、verify/bench bundle writer、JSON+HTML bundle writer 和目录级 report writer，当前已承接报告页面、代码评审 Markdown、CI JUnit 输出、已有 HTML report 复用和 artifact drilldown 链接；后续扩展 GUI report viewer。
...
- GPU schematic canvas renderer：symbol、symbol graphics、schematic-level drawing primitives、embedded image、table/table cell、wire、bus、label、schematic text/SPICE directive text、text box、junction、no-connect、sheet、group/selection metadata、selection、grid；renderer 只消费 `osl-kicad` 的 scene DTO 和 `canvas_hit` selection API，不直接解析 KiCad S-expression。
- 缩放、平移、游标、marker。
- 多曲线叠加。
- sweep 分组显示。
...
- 已新增 `osl-app` / `nekospice` GUI alpha crate，使用 `eframe` 并显式选择 `wgpu` renderer，先复用 Rust-native KiCad canvas scene 和 `KicadCanvasScene::hit_test` 后端实现打开 `.kicad_sch`、基础原理图显示、缩放、平移、fit 和点击选择。GUI 侧只负责视窗与绘制策略，KiCad item bounds / intersection 等几何能力由 `osl-kicad` 暴露并复用；当前已按可见世界 bounds 做 viewport culling，并通过 GUI document adapter 调用 `KicadSchematicEdit::{DeleteItem, MoveItem, PlaceSymbol, AddWire, AddBus, AddBusEntry, AddLabel, AddText, AddJunction, AddNoConnect, AddSheet, SetSymbolProperty, ConfigureSymbol}` 与 KiCad writer 实现选中项删除、方向键/按钮移动、从 library browser 放置 symbol、基础 wire / bus / bus-entry / local-global-hierarchical label / text / junction / no-connect / hierarchical sheet 绘制工具、选中 symbol Reference / Value / mirror 属性编辑和保存闭环。界面已整理为左侧项目/选择状态/属性编辑/绘图工具、中间原理图画布、右侧 symbol library browser 的三栏结构；右侧库浏览器通过 `KicadSymbolLibraryIndex` 加载 `sym-lib-table` 并搜索 symbol 元数据，Place 流程通过 library adapter 读取 `.kicad_sym` 定义和同库继承上下文，支持一次放置、repeat placement、真实 symbol canvas preview、unit/body-style scope controls、pin alternate placement controls，以及画布鼠标位置 placement preview；派生 symbol 放置会把父 symbol definition 一起嵌入 schematic，并保持 KiCad `(extends ...)` 语义，GUI 不直接解析 `.kicad_sym`。编辑后由 `KicadCanvasScene::item_hit_by_uuid` 或同一 hit-test 后端刷新选中状态，避免 GUI 层直接耦合 KiCad S-expression 或几何细节。`osl-app` 已拆分为 app state/event routing、panel composition、selection property editor、schematic tool routing/state/preview、symbol browser / preview、placement config/state、document adapter、library adapter、viewport transforms、canvas drawing、simulation report summary 和 waveform summary 模块，并在 crate README 记录边界。`osl-kicad` 已新增可选外部 KiCad demo 烟测，默认读取本地 `kicad-source-mirror-master/demos` 或 `NEKOSPICE_KICAD_DEMOS`，覆盖 KiCad 仿真 demo、复杂层级 demo 和包含 bus / 大量图形的大型 CM5 demo，验证 `.kicad_pro`、`.kicad_sch`、`sym-lib-table`、本地 `.kicad_sym`、canvas scene、连接图和关键场景特征不会回归，并把 KiCad S-expression 基础层拆入独立 `sexpr` 模块，把 `.kicad_pro` JSON parser / project DTO 拆入独立 `project` 模块，把 `.kicad_sym` library container / `sym-lib-table` container / library root parser 拆入独立 `symbol_library` 模块，把 JSON summary helper 拆入独立 `json` 模块，把 symbol mirror normalization / local transform / rotation helper 拆入独立 `transform` 模块，把 schematic connectivity graph / quantized point / net naming helper 拆入独立 `connectivity` 模块，把 schematic edit command DTO / validation / UUID helper 拆入独立 `edit` 模块，把 GUI-facing symbol library index / query / browser metadata 拆入独立 `library_index` 模块，把 canvas scene DTO / scene 构建拆入独立 `canvas` 模块，把 canvas hit-test / UUID selection refresh 拆入独立 `canvas_hit` 模块，把 canvas bounds / hit-test / arc sampling 等纯几何能力拆入独立 `geometry` 模块，把 diagnostic/check report DTO 与 helper 拆入独立 `diagnostics` 模块，把 title block 元数据拆入独立 `metadata` 模块，把 sheet/symbol/project instance path 与 variant metadata 拆入独立 `instances` 模块，把 schematic summary JSON 统计拆入独立 `schematic_summary` 模块，把 stroke/fill/color/text-effects/margins 样式 IR、样式 parse/write 和样式 JSON 投影拆入独立 `style` 模块，把 point/size/at 坐标 IR、坐标 parse/write 和坐标 JSON 投影拆入独立 `coordinates` 模块；`osl-netlist` 已新增同源 KiCad simulation demo 导入烟测，验证 `.kicad_pro` 解析到 root schematic、SPICE deck 导出、稳定诊断和 normalized project 生成路径，并把 KiCad project/source discovery、LTspice `.asc` importer、通用 SPICE report/normalized project API 拆成独立模块边界，同时不把 KiCad 源码镜像纳入 Git。后续把 egui painter 原型替换为 schematic / waveform 专用 wgpu 批渲染 pipeline，并扩展更完整的 property editing tools。
- 代码管理补充：`osl-kicad` 继续以文件管理和模块边界为第一优先级，已把 schematic image / embedded base64 data、wire/bus/bus-entry/bus-alias/net-chain、junction/no-connect marker、table/table-cell、property、label/directive-label、text/text-box、group、hierarchical sheet/sheet-pin IR、shared drawing primitive / schematic graphic / rule-area IR、symbol pin / placed-symbol pin-ref IR、placed-symbol / symbol-definition IR、symbol 继承、unit/body-style 作用域、library symbol lookup/qualification、symbol 仿真元数据 helper、`.kicad_sch` root parse/write orchestration、schematic-to-SPICE export、hierarchy expansion、SPICE include collection、hierarchy net alias scoping、structured simulation directive discovery/classification/normalization 和 KiCad-compatible directive edit helper 与 parse/write/helper 从 `lib.rs` 拆入 `image.rs`、`wiring.rs`、`markers.rs`、`table.rs`、`property.rs`、`labels.rs`、`text.rs`、`group.rs`、`sheet.rs`、`graphics.rs`、`pins.rs`、`symbols.rs`、`schematic_io.rs`、`spice_export.rs`、`simulation.rs`。当前开发节奏已转为“功能纵切 + 顺手解耦”：新增 `KicadSchematicEdit::SetSimulationDirective`、CLI `set-simulation-directive`、GUI document adapter、独立 `simulation_panel`、`simulation_artifacts_panel`、`simulation_report_panel`、`simulation_waveform_panel` 和 GUI simulation run adapter，GUI 可从同一 document adapter 设置仿真指令、查看 schematic diagnostics / SPICE netlist preview，并把当前 schematic netlist 写入 `runs/gui` 后通过后台任务调用 `osl-sim` 的 ngspice 后端运行，避免长仿真阻塞界面；运行结束统一走 `osl-sim` run artifact finalizer，导出 `waveform.csv` / `waveform-summary.json` / `report.html` 并刷新 `run.json` artifact metadata，让 GUI 和 CLI 运行目录保持一致；批量 verify/bench 的报告 DTO、HTML/JSON/Markdown/JUnit XML renderer、bundle writer、目录级 report writer 和 artifact 链接已从 CLI 拆入 `osl-report`，并由 GUI `report_summary` adapter 消费目录级 summary，开始为后续 GUI report viewer、代码评审摘要和 CI artifact 入口提供共享边界；后台任务完成后由独立 `waveform_summary` adapter 复用 `osl-waveform` 解析 `waveform.raw`，为 GUI 预计算信号统计和固定 bucket 的 waveform preview envelope，Simulation artifact panel 只展示 artifact drilldown，Simulation report panel 只消费 report DTO 显示 HTML/source/mode/size，Simulation waveform panel 只消费 DTO 来显示点数、变量数、关键变量统计、选中信号测量摘要、信号选择和小型波形预览，不直接读取 raw 文件或扫描全量数组。后续拆分继续按“低耦合 IR + parse/write + bounds/helper 同模块”的规则推进，根模块只保留公共 API、顶层装配和暂未拆出的复杂 netlist 逻辑。
...
cargo test -p osl-netlist --test kicad_demo_import_smoke -- --nocapture
```
